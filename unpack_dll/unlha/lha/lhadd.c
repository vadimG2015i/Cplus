...
static char    new_archive_name[FILENAME_LENGTH];

#ifdef WIN32
   /* function prototypes */
   static void add_one(FILE * , FILE * , LzHeader *);
   static void find_update_files(FILE *);
   ...
   static void remove_one(char *);
   static void remove_files(int , char * *);

   extern BOOL g_bMakeBackupFile;
...
//	int             mode;
	...
	LHAwrite_header(nafp, hdr);/* DUMMY */
/*
...
//	int             i;
	...
	if (GETSTAT(name, &stbuf) < 0) {
		LHAerror("Cannot access", name);	/* See cleaning_files, Why? */
		return oafp;
	}

////	dlgSetFileSize(stbuf.st_size, 0); //uema2.
...
//	dlgSetFileSize(stbuf.st_size, stbuf.st_size); //uema2.
...
#ifdef _WIN32_WCE
	wce_AToW2(temporary_name, t);
#else
	_tcscpy(t, temporary_name);
#endif
...
//	old_umask = umask(077);
	...
	if(afp == NULL) fatal_error2(temporary_name, ERROR_TMP_OPEN);
...
build_backup_file()
{
	...
	//	signal(SIGHUP, SIG_IGN);
		if (rename(archive_name, backup_archive_name) < 0)
			fatal_error2(archive_name, ERROR_TMP_BACK_SPACE);
		recover_archive_when_interrupt = TRUE;
/*		// comment-outed by Hanamaru
//		signal(SIGINT, interrupt);
//		signal(SIGHUP, interrupt);
	...
	long            new_archive_size;
{
	...
	if (rename(temporary_name, p) < 0) {
		sprintf(temp, "Can't rename temporary_name '%s'\n", new_archive_name);
		printfToBuffer(temp);
...
//			printf("Removed %s.\n", name);
		...
		LHAerror("Cannot remove (not a file or directory)", name);
	}
}
...
cmd_add()
{
	...
	if (!update_if_newer && cmd_filec == 0) {
		LHAerror("No files given in argument, do nothing.", "");
		return;
	}
	...
	/* exit if cannot execute automatic update */
	if (update_if_newer && cmd_filec == 0 && !oafp)
		fatal_error2(archive_name, ERROR_COMMAND_NAME);	
...
//		new_archive_name = new_archive_name_buffer;
		strcpy (new_archive_name , archive_name);	// Added by Hanamaru
	}
	else {
...
//	dlgSetArchiveName(new_archive_name);
...
//		dlgSetFileName(cmd_filev[i]);
	...
	// ‚±‚±‚ÅAŠù‘¶‚Ì .lzh ‚ð .bak ‚É rename ‚µ‚Ä‚¢‚é
	if (old_archive_exist && g_bMakeBackupFile==TRUE)
		build_backup_file();
	else if(g_bMakeBackupFile==FALSE)
		unlink(new_archive_name);

	report_archive_name_if_different();

	/* copy temporary file to new archive file */
	// temporary ‚Ì lhXXXXXX -> .lzh ‚É rename
	if (!noexec && (!strcmp(new_archive_name, "-") ||
		rename(temporary_name, new_archive_name) < 0)){
		if(FALSE==temporary_to_new_archive_file(new_archive_size))
			return;
	}
...
cmd_delete()
{
	...
	if (archive_is_msdos_sfx1(archive_name)) {
		...
		build_standard_archive_name(new_archive_name_buffer, archive_name);
//		new_archive_name = new_archive_name_buffer;
//		strcpy (new_archive_name , new_archive_name_buffer);
		strcpy (new_archive_name , archive_name);
	}
	else {
//		new_archive_name = archive_name;
//		strcpy (new_archive_name , new_archive_name_buffer);
