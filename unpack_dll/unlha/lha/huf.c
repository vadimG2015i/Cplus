...
#include <stdlib.h>
   ...
   static void write_pt_len(short , short , short);
   static void write_c_len(void);
   static void encode_c(short);
   static void encode_p(unsigned short);
   static void send_block(void);
   static void read_pt_len(short , short , short);
...
unsigned short  c_freq[2 * NC - 1], c_table[4096], c_code[NC], p_freq[2 * LHANP - 1],
	...
	short           i_special;
{
	...
	while (n > 0 && pt_len[n - 1] == 0)
		n--;
	LHAputbits((unsigned char)nbit, n);
	i = 0;
	while (i < n) {
		k = pt_len[i++];
		if (k <= 6)
			...
			LHAputbits((unsigned char)(k - 3), (unsigned short)(USHRT_MAX << 1));
		if (i == i_special) {
...
send_block( /* void */ )
{
	...
	for (i = 0; i < size; i++) {
		...
		if (flags & (1 << (CHAR_BIT - 1))) {
			encode_c((short)(buf[pos++] + (1 << CHAR_BIT)));
			k = buf[pos++] << CHAR_BIT;
			k += buf[pos++];
			encode_p(k);
		} else
...
alloc_buf( /* void */ )
{
	...
	while ((buf = (unsigned char *)LocalAlloc( LPTR, bufsiz)) == NULL) {
		bufsiz = (bufsiz / 10) * 9;
		if (bufsiz < 4 * 1024)
			break;
	}
...
encode_end_st1( /* void */ )
{
	if (!unpackable) {
		send_block();
		LHAputbits(CHAR_BIT - 1, 0);	/* flush remaining bits */
	}
}
