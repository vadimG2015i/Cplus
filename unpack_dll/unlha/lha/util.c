...
#ifdef WIN32
   /* function prototypes */
   static int my_toupper(register int);
   extern void init_code_cache();
				 ...
				 * append */
{
	...
	if ((buf = (char *)LocalAlloc( LPTR, BUFFERSIZE+1)) == NULL)
	...
	while (size > 0) {
		...
		if (crc_flg == 3 && text_mode) {
			...
			xsize = (size > BUFFERSIZE) ? BUFFERSIZE : (unsigned short)size;
...
//	unsigned char   buffer[MAXSFXCODE];
	char *buffer;			// Added by Hanamaru
	unsigned char  *p, *q;
	int             n;

	if ((buffer = LocalAlloc( LPTR, MAXSFXCODE)) == NULL)
	...
	char           *buf;
{
	...
	if ((p = (char *)LocalAlloc( LPTR, strlen(buf) + 1)) == NULL)
		return NULL;
	strcpy(p, buf);
	return p;
}
	...
	char           *path;
{
	int             stat, rtn = 0;
	char           *cmdname;
	if ((cmdname = (char *)LocalAlloc( LPTR, 
		strlen(RMDIRPATH) + 1 + strlen(path) + 1)) == 0)
		return (-1);
