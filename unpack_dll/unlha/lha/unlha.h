...
#if !defined(UNLHACE_H)
#define UNLHACE_H

#define UNLHACE_VERSION		92
...
WORD _export  UnlhaGetVersion(VOID);

BOOL _export UnlhaGetRunning(VOID);
/*
...
*/
int _export Unlha(const HWND _hwnd, LPCTSTR _szCmdLine,
						LPTSTR _szOutput, const DWORD _dwSize);
...
BOOL _export UnlhaCheckArchive(LPCTSTR _szFileName, const int _iMode);
/*
...
// 4CE では SubVersion を導入する予定なし
//WORD WINAPI _export UnlhaGetSubVersion(VOID);

/*int _export UnlhaExtractMem(const HWND _hwndParent,
		LPCTSTR _szCmdLine, LPBYTE _lpBuffer, const DWORD _dwSize,
