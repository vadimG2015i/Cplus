...
#include "lha.h"
   ...
   void free_hashbuf(void);

   extern void writeToMem(unsigned char *p, unsigned long lc);
   extern BOOL g_bExtractMem;
...
//	if (alloc_buf() == NULL) exit(207); /* I don't know this 207. */
	if (alloc_buf() == NULL) 
		fatal_error2("Code:207", ERROR_MORE_HEAP_MEMORY);

	hash = (unsigned int*)LocalAlloc( LPTR, HSHSIZ * sizeof(unsigned int));
	prev = (unsigned int*)LocalAlloc( LPTR, DICSIZ * sizeof(unsigned int));
	text = (unsigned char*)LocalAlloc( LPTR, TXTSIZ);
	too_flag = (unsigned char*)LocalAlloc( LPTR, HSHSIZ);

	if (hash == NULL || prev == NULL || text == NULL || too_flag == NULL)
		fatal_error2("Code:207\n", ERROR_MORE_HEAP_MEMORY);
//		exit(207);
...
#endif
	n = LHAfread_crc(&text[(unsigned)(txtsiz - dicsiz)], 
...
static void match_insert()
{
	...
	off = 0;
	for (h = hval; too_flag[h] && off < (unsigned int)(maxmatch - THRESHOLD); ) {
		h = ((h << 5) ^ text[pos + (++off) + 2]) & (unsigned)(HSHSIZ - 1);
	}
	if (off == (unsigned int)(maxmatch - THRESHOLD)) off = 0;
	for (;;) {
		chain = 0;
		scan_pos = hash[h];
		...
		while (scan_pos > scan_end) {
			...
			if (text[scan_pos + matchlen - off] == text[pos + matchlen]) {
				...
				if (len > (unsigned int)matchlen) {
					matchpos = scan_pos - off;
					if ((matchlen = len) == (int)max) {
						break;
					}
#ifdef _DEBUG
					if (noslide) {
...
//	if (fout == NULL) exit(1);
	if (fout == NULL){
		fatal_error("slide/encode() error");
	}
#endif
	...
	remainder = LHAfread_crc(&text[dicsiz], txtsiz-dicsiz, infile);
	encoded_origsize = remainder;
	matchlen = THRESHOLD - 1;

	pos = dicsiz;

	if (matchlen > (int)remainder) matchlen = remainder;
	...
	struct interfacing *interface;
{
	...
	fout = fopen("de", "wt");
	if (fout == NULL)
		fatal_error("slide/decode() error");
#endif
	...
	prev_char = -1;
	dicsiz = 1L << dicbit;
	dtext = (unsigned char *)LocalAlloc( LPTR, dicsiz );
    if (dtext == NULL){
		char temp[10]={0};
		/* error(MEMOVRERR, NULL); */
		sprintf(temp, "%d", errno);
		fatal_error2(temp, ERROR_ENOUGH_MEMORY);
	}
...
//				fwrite_crc(dtext, dicsiz, outfile);
				if(g_bExtractMem)
					writeToMem(dtext, loc);
				else
					LHAfwrite_crc(dtext, loc, outfile);
				loc = 0;
			}
			count++;
		}
