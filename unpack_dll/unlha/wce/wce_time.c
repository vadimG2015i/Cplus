...
static __int64 wce_GetDeltaSecs(FILETIME f1, FILETIME f2)
{
	...
	__int64 t1, t2, iTimeDiff;
		
	t1 = f1.dwHighDateTime;
	t1 <<= 32;				
	t1 |= f1.dwLowDateTime;

	t2 = f2.dwHighDateTime;
	t2 <<= 32;				
	t2 |= f2.dwLowDateTime;
...
static void wce_GetTZBias(int* pTZBiasSecs, 
						  int* pDSTBiasSecs)
{
...
//time_t wce_mktime(struct tm* pt)
time_t mktime(struct tm* pt)
{
	SYSTEMTIME s;
	FILETIME f;
...
//	static const FILETIME f1970 = wce_YearToFileTime(1970);
	static FILETIME f1970;
	
	f1970 = wce_YearToFileTime(1970);

	// Convert input tm to SYSTEMTIME
	s = wce_TmToSystemTime(pt);
...
//tm* wce_localtime(const time_t *ptime)
struct tm* localtime(const time_t *ptime)
{
...
//		wce_GetDeltaSecs(wce_YearToFileTime(1601), wce_YearToFileTime(1970));
	...
	iOffset = wce_GetDeltaSecs(wce_YearToFileTime(1601), wce_YearToFileTime(1970));
...
//	struct tm *ptime = wce_localtime(ulSecsSince1970);
	struct tm *ptime = localtime(ulSecsSince1970);
