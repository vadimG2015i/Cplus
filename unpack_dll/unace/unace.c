...
INT  read_headerAce(INT print_err)         // reads any header from archive
{
   USHORT rd,
        head_size,
        crc_ok;
   ...
   while (lseek(archan, 0, SEEK_CUR)<st.st_size && fpos < max_sfx_size)
   {
      ...
      {                         
         ...
         {             
            ...
            if (read_headerAce(0))         // try to read archive header
            {                           
...
INT  proc_next_vol(void)        // opens next volume to process
{
   ...
   if (!proc_vol())             // try to open volume, read archive header
      return 0;
   if (!read_headerAce(1))         // read 2nd header
   {
      f_err=ERR_READ;
      return 0;
   }
...
INT  read_adds_blkAce(CHAR * buffer, INT len)      // reads part of ADD_SIZE block
{
   INT  rd = 0,
        l = len;
   LONG i;
...
int AceInit(INT argc, CHAR * argv[])              // processes the archive
{
   INT show_help,
       arg_cnt = 1;
