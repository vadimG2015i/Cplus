...
void readdat(void)
{
   ...
   read_adds_blkAce((CHAR *) & buf_rd[2], i);
...
INT unstoreAce(CHAR * buf, UINT len)
{
   UINT rd = 0,
        i,
        pos = 0;

   while ((i = read_adds_blkAce((CHAR *) buf_rd, (INT) ((i = ((len > dcpr_size) ? dcpr_size : len)) > size_rdb ? size_rdb : i))) != 0)
   {
      rd += i;
      len -= i;
      dcpr_size -= i;
...
void dcpr_init_file(void)
{
   ...
   if (fhead.TECH.TYPE == TYPE_LZ1)
   {
      ...
      read_adds_blkAce((CHAR *) buf_rd, i);
#ifdef HI_LO_BYTE_ORDER
      {
         ULONG *p;
         i>>=2;    // count LONGs not BYTEs
