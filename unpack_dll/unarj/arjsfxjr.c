/*
 * $Id: arjsfxjr.c,v 1.1.1.1 2002/03/28 00:01:23 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static void find_sfx_header()
{
 ...
 do
 {
  ...
  if((basic_hdr_size=fget_word())<=HEADERSIZE_MAX)
  {
   ...
   if((crc32term^CRC_MASK)==fget_longword())
   {
    _lseek(aistream, (long)exe_pos, SEEK_SET);
    return;
   }
...
static int read_header()
{
 ...
 crc32term=CRC_MASK;
 fread_crc(header, basic_hdr_size);
 if((header_crc=fget_longword())!=(crc32term^CRC_MASK))
 ...
 ftime_stamp=*(unsigned long *)&header[8];
 compsize=*(unsigned long *)&header[12];
 origsize=*(unsigned long *)&header[16];
 file_crc=*(unsigned long *)&header[20];
 entry_pos=*(unsigned short *)&header[24];
 file_mode=(unsigned int)*(unsigned short *)&header[26];
...
static char *preprocess_comment(char *comment)
{
  ...
  *aptr='\0';
  endptr=aptr;
  aptr=cmd_args;
  while((unsigned int)aptr<(unsigned int)endptr)
  {
   while(*aptr=='\0')
    aptr++;
   if((unsigned int)aptr<(unsigned int)endptr)
   {
    arg_rc=analyze_arg(aptr);
    while(*aptr!='\0'&&(unsigned int)aptr<(unsigned int)endptr)
     aptr++;
   }
  }
  if(arg_rc)
...
void main(int argc, char **argv)
{
 ...
 exit((warnings>0)?ARJSFXJR_ERL_ERROR:ARJSFXJR_ERL_SUCCESS);
}
...
void read_c_len()
{
 ...
 else
 {
  ...
  while(i<NCC)
   c_len[i++]=0;
  make_table(NCC, c_len, 12, c_table, sizeof(c_table));
 }
}
