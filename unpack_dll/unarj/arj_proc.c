/*
 * $Id: arj_proc.c,v 1.3 2002/05/14 22:03:53 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static
int *os2sw_index[]={&os2switch, &arcmail_sw, &crit_eas, &dos_host,
                    &include_eas, &disable_comment_series, &suppress_hardlinks,
                    &skip_century, &fix_longnames, &do_chown,
                    &priority.class1, &symlink_accurancy, &exclude_eas, NULL};
...
int *sw_index[]={&skip_integrity_test, &prompt_for_directory, &skip_ts_check,
                 ...
                 &arjdisp_enabled, &execute_extr_cmd, NULL};
static
...
void append_curtime_proc()
{
  ...
  *aptr='\0';
  lim=0;
  for(tptr=time_str+l-1; (unsigned int)tptr>=(unsigned int)time_str; tptr--)
  {
   if(*tptr==*(tptr+1))
...
void parse_arj_sw(int cmd, char *arj_sw, char *dest)
{
 ...
 dptr=use_file?arj_sw:arj_env_name;
 msg_sprintf(misc_buf, M_USING_ENV_VAR, dptr, sw);
 scroll_puts(misc_buf);
 /* Tokenize switch */
 ...
 sptr=dptr;
 dptr=sw;
 while((unsigned int)dptr<(unsigned int)sptr)
 {
  while(*dptr=='\0')
   dptr++;
  if((unsigned int)dptr<(unsigned int)sptr)
  {
   if(is_switch(dptr))
    analyze_arg(dptr);
   while(*dptr!='\0'&&(unsigned int)dptr<(unsigned int)sptr)
    dptr++;
  }
 }
...
unsigned int get_volfree(unsigned int increment)
{
 ...
 return((unsigned int)min(remain, (unsigned long)increment));
}
...
int check_integrity(char *name)
{
 ...
 strcpy(pattern, integrity_pattern);
 pattern[0]--;
 ...
 else
 {
  ...
  while((c=(unsigned int)*(swptr++))!='\0')
  {
   ...
   if(entry==NULL)
   {
     ...
     msg_printf(M_INVALID_SWITCH, (char)c);
//     check_fmsg(CHKMSG_NOSKIP);
   ...
   /* ARJ parameters */
    ...
    else if(!jswitch&&!hswitch&&!os2switch&&c=='E'&&sw_tail)
    {
     ...
     left_trim=(unsigned int)strtoul(swptr, &swptr, 10);
    ...
    else if(!jswitch&&!hswitch&&!os2switch&&c=='T'&&sw_tail)
    {
     ...
     {
      swptr++;
      type_override=(type_override==FT_TEXT)?FT_TEXT_GRAPHICS:FT_TEXT_FORCED;
     }
     if(*swptr=='.')
     {
      swptr_t=swptr;
    ...
    else if(!jswitch&&!hswitch&&!os2switch&&c=='V'&&sw_tail)
    {
     ...
     while(*swptr!='\0')
     {
      ...
      else if(vol_sw=='P')
      {
       ...
       if((change_vol_delay=(int)strtoul(swptr, &swptr, 10))==0)
        change_vol_delay=STD_CHANGE_VOL_DELAY;
      }
      else if(vol_sw=='R')
      {
       swptr++;
       mv_reserve_space=strtoul(swptr, &swptr, 10);
       if(toupper(*swptr)=='K')
        mv_reserve_space*=1000L;
       else if(toupper(*swptr)=='M')
        mv_reserve_space*=1000000L;
      }
      ...
      {
       volume_limit=strtoul(swptr, &swptr, 10);
       if(toupper(*swptr)=='K')
        volume_limit*=1000L;
       else if(toupper(*swptr)=='M')
        volume_limit*=1000000L;
       break;
      }
      else
       error(M_INVALID_SWITCH, sw);
    ...
    else if(hswitch&&c=='M')
    {
     ...
     max_filenames=(FILE_COUNT)strtoul(swptr, &swptr, 10);
     if(max_filenames==0)
    ...
    else if(jswitch&&c=='C'&&sw_tail)
    {
     ...
     exit_after_count=1;
     exit_count=(FILE_COUNT)strtoul(swptr, &swptr, 10);
    }
    else if(jswitch&&c=='D'&&sw_tail)
    {
     done=1;
     chk_free_space=1;
     minfree=strtoul(swptr, &swptr, 10);
     if(toupper(*swptr)=='K')
      minfree*=1000L;
     else if(toupper(*swptr)=='M')
      minfree*=1000000L;
    ...
    else if(jswitch&&c=='B'&&sw_tail)
    {
     ...
     {
      cnv_num=strtoul(swptr, &swptr, 10);
    ...
    else if(jswitch&&c=='X'&&sw_tail)
    {
     ...
     start_at_ext_pos=1;
     ext_pos=strtoul(swptr, &swptr, 10);
     /* For long ext_pos, we need to put check for ext_pos<0L here, but we omit
        it since in our implementation, ext_pos is unsigned. */
     if(toupper(*swptr)=='K')
      ext_pos*=1000L;
    }
    ...
    else if(os2switch&&c=='P')
    {
     if(!sw_tail)
      error(M_INVALID_SWITCH, sw);
     priority.class1=strtoul(swptr, &swptr, 10);
     if(priority.class1<1||priority.class1>PRIORITY_CLASSES)
      error(M_INVALID_SWITCH, sw);
     #if TARGET==OS2
      ...
      {
       swptr=p_ptr+1;
       priority.delta=strtoul(swptr, &swptr, 10);
   ...
   /* ARJSFXV parameters */
    ...
    else if(c=='#'&&sw_tail)
    {
     ...
     skip_volumes=1;
     first_volume_number=strtoul(swptr, &swptr, 10);
    }
    else if(c=='O'&&sw_tail)
    {
     done=1;
...
void init()
{
 ...
 debug_enabled=0;
  ...
  arcmail_sw=0;
  dos_host=0;
  priority.class1=priority.delta=0;
 #endif
 arjdisp_enabled=0;
 #if SFX_LEVEL<=ARJSFXV
  subdir_extraction=0;
 ...
 av_total_longnames=0;
  ...
  #if TARGET==UNIX
   hl_entries.list=NULL;
   hl_entries.total=hl_entries.alloc=0;
  #endif
 #endif
...
static int get_str_from_jq()
{
 ...
 ignore_pcase=sptr[0]=='+';
 fdisp_lines=(int)strtoul(sptr, &sptr, 10);
 patterns=0;
 if(*sptr!='\0')
 {
  pt=*sptr;
...
void search_setup()
{
 ...
 else
 {
  ...
  msg_printf(M_ENTER_NUM_MATCHES);
//  read_line(entry, sizeof(entry));
  fdisp_lines=strtoul(entry, NULL, 0);  /* Can accept hex values */
  msg_printf(M_ENTER_SEARCH_STR, SEARCH_STR_MAX);
  for(patterns=0; patterns<SEARCH_STR_MAX; patterns++)
  {
   fprintf(new_stdout, le_prompt, patterns+1);
