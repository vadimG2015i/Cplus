/*
 * $Id: decode.c,v 1.1.1.1 2002/03/28 00:02:16 andrew_belov Exp $
 * ---------------------------------------------------------------------------
  ...
  if(compsize>0)
  {
   ...
   if(file_packing)
   {
    /* This slows the things down quite a lot so we won't put this in the
     ...
     bbrc=fgetc(aistream);
     if(errno!=0)
      msg_printf(M_DECODE_CRIT_ERROR);
     if(bbrc<0)
      msg_printf(M_DECODE_EOF);
...
#endif
{
 ...
 if(start[17]!=(unsigned short)(1<<16))
 {
  if(file_garbled)
  {
   delay(BADTABLE_G_DELAY);
   #if SFX_LEVEL>=ARJSFXV
    msg_fprintf(new_stdout, M_BADTABLE_G);
   #else
    error(M_BADTABLE_G);
   #endif
  }
   ...
   #if SFX_LEVEL>=ARJSFXV
    msg_fprintf(new_stdout, M_BADTABLE);
   #else
    error(M_BADTABLE);
   #endif
  #if SFX_LEVEL>=ARJSFXV
...
void read_c_len()
{
 ...
 else
 {
  ...
  while(i<NCC)
   c_len[i++]=0;
  make_table(NCC, c_len, 12, c_table, CTABLESIZE);
 }
}
