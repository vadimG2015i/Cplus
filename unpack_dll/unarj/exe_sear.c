/*
 * $Id: exe_sear.c,v 1.1.1.1 2002/03/28 00:02:43 andrew_belov Exp $
 * ---------------------------------------------------------------------------
 ...
 while(1)
 {
  ...
  while(i<bytes_read)
  {
   if(*buf_ptr=='A'&&!strcmp(buf_ptr+1, overlay_sig))
   {
    ...
    if(!memcmp(buf_ptr+8, &sig_pos, min(4, sizeof(unsigned long))))
     goto loc_hdr;
   }
   i++;
   buf_ptr++;
...
static void fetch_block(int num)
{
 ...
 farblock_output(aostream, (char FAR *)&desc_word, 2L);
 if(is_registered)
  desc_word=REG_ID;
 farblock_output(aostream, (char FAR *)&desc_word, 2L);
 crc32term^=CRC_MASK;
 farblock_output(aostream, (char FAR *)&crc32term, 4L);
}
