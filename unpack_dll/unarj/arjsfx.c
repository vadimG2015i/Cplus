/*
 * $Id: arjsfx.c,v 1.1.1.1 2002/03/28 00:01:23 andrew_belov Exp $
 * ---------------------------------------------------------------------------
 ...
 if(!logo_shown)
 {
  for(nptr=tptr=archive_name; *tptr!='\0'; tptr++)
   if(strchr(path_separators, *tptr)!=NULL)
    nptr=tptr+1;
  //msg_sprintf(misc_buf, M_ARJSFX_BANNER, M_VERSION, nptr, build_date);
...
void main(int argc, char *argv[])
{
...
#endif
 {
  if(licensed_sfx)
  {
   msg_printf(M_EXECUTING_CMD, extr_cmd_text);
   delay(2);
  ...
  else
  {
    ...
    if(query_action(REPLY_YES, QUERY_CRITICAL, (FMSG *)misc_buf))
     exec_command(extr_cmd_text);
   #else
    msg_printf(M_EXTR_CMD_QUERY, extr_cmd_text);
    if(query_action())
     exec_command(extr_cmd_text);
   #endif
  }
