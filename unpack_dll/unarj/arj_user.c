/*
 * $Id: arj_user.c,v 1.2 2002/05/14 22:03:53 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
#if SFX_LEVEL>=ARJ
static jmp_buf main_proc;               /* Entry point of archive processing
                                           loop */
#endif
...
/* Outputs text and returns directly to the main processing loop once the user
   cancels the output */
...
void arj_user_msg(FMSG *text)
{
 ...
 #define arj_user_msg scroll_puts
#else
 #define arj_user_msg msg_printf
#endif
...
static void show_list_header()
{
  ...
  if(std_list_cmd)
  {
   ...
   if(verbose_display==VERBOSE_ENH)
    arj_user_msg(M_VLIST_P1);
   else
   {
    arj_user_msg(M_VLIST_HDR);
    arj_user_msg(M_VLIST_P1);
   }
  }
  else
   arj_user_msg(M_LIST_P1);
  arj_user_msg(verbose_display?M_LIST_P2_CHAP:M_LIST_P2);
 #else
  if(std_list_cmd)
  {
   arj_user_msg(M_VLIST_HDR);
   ...
   arj_user_msg(M_LIST_P1);
  arj_user_msg(M_LIST_P2);
 #endif
 arj_user_msg(M_LIST_SEPARATOR);
}
...
#endif
{
 ...
 if(std_list_cmd)
 {
   ...
   if(verbose_display!=VERBOSE_ENH)
   {
    ...
    {
  #endif
     msg_sprintf(misc_buf, M_LIST_NUM_FMT, fnum);
     arj_user_msg((FMSG *)misc_buf);
  #if SFX_LEVEL>=ARJ
    }
  #endif
    #if SFX_LEVEL>=ARJ
     entry=(exclude_paths==EP_PATH)?(unsigned int)entry_pos:0;
     msg_sprintf(misc_buf, M_FILENAME_FORM, tmp_name+entry);
    #else
     entry=0;
     msg_sprintf(misc_buf, M_FILENAME_FORM, filename);
    #endif
    arj_user_msg((FMSG *)misc_buf);
    #if SFX_LEVEL>=ARJ
     if(verbose_display==VERBOSE_STD)
    ...
    if(comment[0]!='\0')
    {
     ...
     #if SFX_LEVEL>=ARJ
      arj_user_msg((FMSG *)lf);
     #endif
    }
    #if SFX_LEVEL>=ARJ
     if(ext_flags!=0)
     {
      msg_sprintf(misc_buf, M_CHAPTER_LIST_FMT, (int)ext_flags, (int)chapter_number);
      arj_user_msg((FMSG *)misc_buf);
     }
    #endif
  #if SFX_LEVEL>=ARJ
   ...
   msg_sprintf(misc_buf, M_REV_OS_FMT, (int)arj_nbr, tmp_ptr);
  #else
   msg_printf(M_VERBOSE_NAME_FMT, filename);
   if(comment[0]!='\0')
   {
   ...
   msg_printf(M_REV_OS_FMT, (int)arj_nbr, host_os_names[host_os]);
  #endif
 }
 else
  #if SFX_LEVEL>=ARJ
   msg_sprintf(misc_buf, (strlen(tmp_name+entry_pos)>12)?M_LONG_NAME_FMT:M_SHORT_NAME_FMT, tmp_name+entry_pos);
  #elif SFX_LEVEL>=ARJSFXV
   msg_sprintf(misc_buf, (strlen(filename+entry_pos)>12)?M_LONG_NAME_FMT:M_SHORT_NAME_FMT, filename+entry_pos);
  #else
   msg_printf((strlen(list_adapted_name)>12)?M_LONG_NAME_FMT:M_SHORT_NAME_FMT, list_adapted_name);
  #endif
 #if SFX_LEVEL>=ARJSFXV
  arj_user_msg((FMSG *)misc_buf);
 #endif
#if SFX_LEVEL>=ARJ
 if(verbose_display)
  msg_sprintf(misc_buf, M_VERBOSE_LIST_LINE, origsize, compsize, ratio/1000, ratio%1000, pick_century(timetext), timetext+2, (int)ext_flags, (int)chapter_number, mode_str, LFLAGS);
 else
#endif
 #if SFX_LEVEL>=ARJSFXV
  msg_sprintf(misc_buf, M_STD_LIST_LINE, origsize, compsize, ratio/1000, ratio%1000, pick_century(timetext), timetext+2, LMODESTR, LFLAGS);
  arj_user_msg((FMSG *)misc_buf);
 #else
  msg_printf(M_STD_LIST_LINE, origsize, compsize, ratio/1000, ratio%1000, timetext+2, LMODESTR, LFLAGS);
 #endif
 #if SFX_LEVEL>=ARJ
  if(std_list_cmd&&verbose_display==VERBOSE_ENH)
  {
   if((tmp_ptr=strrchr(tmp_name, '.'))==NULL)
    tmp_ptr=nullstr;
   msg_sprintf(misc_buf, M_PATH_LIST_FMT, tmp_ptr, tmp_name+entry_pos, filename);
   arj_user_msg((FMSG *)misc_buf);
  }
  arj_user_msg((FMSG *)lf);
  if(std_list_cmd&&ts_valid(atime_stamp)&&verbose_display!=VERBOSE_ENH)
  {
   ...
   msg_sprintf(misc_buf, M_ATIME_FMT, pick_century(timetext), timetext+2);
   arj_user_msg(misc_buf);
   timestamp_to_str(timetext, &ctime_stamp);
   msg_sprintf(misc_buf, M_CTIME_FMT, pick_century(timetext), timetext+2);
   arj_user_msg(misc_buf);
  }
  ...
  if(std_list_cmd&&valid_ext_hdr&&!(arj_flags&VOLUME_FLAG)&&verbose_display!=VERBOSE_ENH)
  {
   ...
   if((p_eh=eh_lookup(eh, EA_ID))!=NULL&&(!file_garbled||garble_enabled))
   {
    ...
    msg_sprintf(misc_buf, M_EA_LIST, raw_ea_size, p_eh->size, ratio/1000, ratio%1000, get_num_eas(raw_ea));
    arj_user_msg(misc_buf);
    farfree(raw_ea);
   }
  }
...
static void process_archive(int cmd, int no_in_arch)
{
 ...
 while(!no_in_arch&&read_header(0, aistream, archive_name))
 {
  ...
  if(origsize>4294967296) break;
  if (lpFreeBytesAvailable.QuadPart<origsize) break;
   ...
   case ARJ_CMD_LIST:
    if(pf_num!=0&&val_result)
    {
	    ...
	    datafile->ArjFile=malloc(strlen(filename));
		OemToChar(filename,datafile->ArjFile);
	    strcpy(datafile->ArjFile,filename);
	    datafile->timefile=ftime_stamp;
	    node=Create_Node(listA, datafile);
...
static void finish_processing(int cmd)
{
 ...
 if(modify_command)
 {
  ...
  {
   msg_printf(M_DELETING_EMPTY_ARCH, archive_name);
   file_close(aistream);
   aistream=NULL;
   if(!no_file_activity)
   {
  ...
  else if(total_files!=0||(cmd==ARJ_CMD_COMMENT&&comment_entries!=0)||create_sfx!=0)
  {
   ...
   if(!no_file_activity)
   {
    ...
    if(test_archive_crc&&protected)
    {
     ...
     {
      msg_printf(M_CANT_FIND_DAMAGE, archive_name);
      printf(lf);
     }
     else
     {
   ...
   if(!no_file_activity)
   {
    ...
    {
     msg_printf(M_COPYING_TEMP, tmp_archive_name, archive_name);
     tmp_archive_used=1;
 ...
 if(cmd==ARJ_CMD_LIST)
 {
  if(!std_list_cmd||verbose_display!=VERBOSE_ENH)
  {
   if(total_files==0||(std_list_cmd&&verbose_display==VERBOSE_STD))
   {
    if(total_longnames==0)
     msg_sprintf(misc_buf, M_N_FILES, total_files);
    else
     msg_sprintf(misc_buf, M_N_FILES_LFN, total_files, total_longnames);
    arj_user_msg((FMSG *)misc_buf);
    if(total_chapters!=0)
    {
     arj_user_msg(M_CHAPTERS_ON);
     arj_user_msg((FMSG *)lf);
    }
   }
   else
   {
    arj_user_msg(M_BRIEF_LIST_SEPARATOR);
    if(total_chapters!=0)
    {
    ...
    if(total_longnames==0)
     msg_sprintf(misc_buf, M_TOTAL_STATS, total_files, total_uncompressed, total_compressed, ratio/1000, ratio%1000, msg_ptr);
    else
     msg_sprintf(misc_buf, M_TOTAL_STATS_LFN, total_files, total_uncompressed, total_compressed, ratio/1000, ratio%1000, msg_ptr, total_longnames);
    arj_user_msg((FMSG *)misc_buf);
    if(av_total_files>total_files)
    {
     ratio=calc_percentage(av_compressed, av_uncompressed);
     if(total_longnames==0)
      msg_sprintf(misc_buf, M_TOTAL_STATS, av_total_files, av_uncompressed, av_compressed, ratio/1000, ratio%1000, nullstr);
     else
      msg_sprintf(misc_buf, M_TOTAL_STATS_LFN, av_total_files, av_uncompressed, av_compressed, ratio/1000, ratio%1000, nullstr, av_total_longnames);
     arj_user_msg((FMSG *)misc_buf);
    }
   }
  }
  if(chk_free_space)
  {
   ...
   {
    msg_printf(M_NOT_ENOUGH_SPACE_X, disk_space_used+minfree-free_space);
    errors++;
   }
  }
 }
 ...
 {
  if(!help_issued)
   msg_fprintf(new_stdout, M_N_FILES, total_files);
 }
 else if(cmd==ARJ_CMD_COMMENT)
  msg_printf(M_N_COMMENTS, comment_entries);
 else if(cmd==ARJ_CMD_ADD||cmd==ARJ_CMD_FRESHEN||cmd==ARJ_CMD_UPDATE)
 {
  if(total_files==0)
  {
   if(total_longnames==0)
    msg_printf(M_N_FILES, total_files);
   else
    msg_printf(M_N_FILES_LFN, total_files, total_longnames);
  }
  else
  {
   if(verbose_display==VERBOSE_STD)
   {
    arj_user_msg(M_FINAL_FOOTER);
    ratio=calc_percentage(total_compressed, total_uncompressed);
    msg_printf(M_VERBOSE_FOOTER, total_files, total_uncompressed, total_compressed, ratio/10, ratio%10);
    if(total_files<av_total_files)
    {
     ratio=calc_percentage(av_compressed, av_uncompressed);
     msg_printf(M_VERBOSE_FOOTER, av_total_files, av_uncompressed, av_compressed, ratio/10, ratio%10);
    }
   }
   else
   {
 ...
 else
 {
  ...
  {
   if(chapter_mode==CHAP_USE)
    scroll_puts(M_CHAPTERS_ON);
   else if(chapter_mode==CHAP_REMOVE)
    scroll_puts(M_CHAPTERS_OFF);
   scroll_puts((FMSG *)lf);
  }
  if(cmd==ARJ_CMD_COPY&&protfile_option&&!arjprot_tail)
   arj_user_msg(M_ARJPROT_DISABLED);
  msg_sprintf(misc_buf, M_N_FILES, total_files);
  scroll_puts((FMSG *)misc_buf);
  if(comment_entries>0)
  {
   msg_sprintf(misc_buf, M_N_COMMENTS, comment_entries);
   scroll_puts((FMSG *)misc_buf);
  }
 }
 if(security_state==ARJSEC_SIGNED&&arjsec_opt!=ARJSECP_SKIP)
 {
  msg_sprintf(misc_buf, M_VALID_ARJSEC, arjsec_signature);
  arj_user_msg((FMSG *)misc_buf);
  msg_strcpy(strcpy_buf, M_SDN_1);
  if(strstr(arjsec_signature, strcpy_buf)!=NULL)
  {
   msg_strcpy(misc_buf, M_SDN_ADD_DESC);
   arj_user_msg((FMSG *)misc_buf);
   msg_strcpy(misc_buf, M_SDN_SECURITY_TEST);
   arj_user_msg((FMSG *)misc_buf);
  }
  msg_strcpy(strcpy_buf, M_SDN_2);
  if(strstr(arjsec_signature, strcpy_buf)!=NULL)
  {
   msg_strcpy(misc_buf, M_SDN_DIST_DESC);
   arj_user_msg((FMSG *)misc_buf);
   msg_strcpy(misc_buf, M_SDN_SECURITY_TEST);
   arj_user_msg((FMSG *)misc_buf);
  }
 ...
 if(arjprot_tail)
 {
  msg_strcpy(misc_buf, M_ARJPROT_ENABLED);
  arj_user_msg(misc_buf);
 }
...
static int process_archive_proc(int cmd)
{
 ...
 if(!setjmp(main_proc))
 {
  ...
  if((tmp_ptr=strchr(debug_opt, 'v'))!=NULL)
   msg_fprintf(new_stdout, M_BRIEF_MEMSTATS, coreleft(), t_buf, v_buf);
  if(chk_free_space)
   ...
   if(modify_command)
   {
    do
    {
     ...
     if(debug_enabled&&strchr(debug_opt, 'v')!=NULL)
      msg_fprintf(new_stdout, M_AVAIL_SPACE, avail_space);
     if(avail_space<MIN_VOLUME_SIZE)
     {
      msg_printf(M_FILENAME_FORM, archive_name);
      if(!yes_on_all_queries&&!skip_space_query)
      {
       msg_printf(M_NOT_ENOUGH_SPACE_V);
       return(0);
      }
      else
  ...
  if(aistream==NULL&&msg_strchr(M_UPDATE_COMMANDS, (char)cmd)==NULL)
  {
   ...
   {
    error_freport();
    msg_fprintf(new_stdout, M_CANTOPEN, archive_name);
    nputlf();
    return(0);
   }
   else
  ...
  if(!modify_command)
  {
//   msg_sprintf(misc_buf, M_PROCESSING_ARCHIVE, archive_name);
  ;// arj_user_msg((FMSG *)misc_buf);
  }
  else
  {
   ...
   if(create_sfx)
   {
    if(!first_vol_passed)
    {
     ...
     msg_printf(M_CREATING_SFX, sfx_name);
     free(sfx_name);
    }
   }
   else
    msg_printf((aistream==NULL)?M_CREATING_ARCHIVE:M_UPDATING_ARCHIVE, archive_name);
   if(!no_file_activity)
   {
    if(aistream==NULL)
    {
    ...
    if(volume_limit>avail_space)
    {
     if(!yes_on_all_queries&&!skip_space_query)
     {
      msg_printf(M_FILENAME_FORM, tmp_archive_name);
      msg_sprintf(misc_buf, M_LOW_SPACE_WARNING, avail_space);
      if(!query_action(REPLY_YES, QUERY_LOW_SPACE, (FMSG *)misc_buf))
      {
       tmp_archive_cleanup();
  ...
  if(aistream==NULL)
  {
   ...
   if(win32_platform&&use_ansi_cp)
    arj_user_msg(M_ANSI_CP_ARCHIVE);
  }
  else
  {
   if((main_hdr_offset=find_header(0, aistream))<0L)
   {
    msg_sprintf(misc_buf, M_NOT_ARJ_ARCHIVE, archive_name);
    arj_user_msg((FMSG *)misc_buf);
    arj_user_msg((FMSG *)lf);
    file_close(aistream);
    aistream=NULL;
   ...
   {
    msg_fprintf(new_stdout, M_MV_UPDATE_REQ_SW);
    file_close(aistream);
    aistream=NULL;
    errors++;
    tmp_archive_cleanup();
   ...
   timestamp_to_str(timetext, &ftime_stamp);
   msg_sprintf(misc_buf, M_ARCHIVE_CREATED, timetext);
   arj_user_msg((FMSG *)misc_buf);
   if(arj_nbr>=ARJ_M_VERSION)
   {
    ts_store(&tmp_time, host_os, compsize);
    timestamp_to_str(timetext, &tmp_time);
   // msg_sprintf(misc_buf, M_MODIFIED, timetext);
    //arj_user_msg((FMSG *)misc_buf);
    if(total_chapters!=0)
    {
     //msg_sprintf(misc_buf, M_CHAPTER_NUMBER, total_chapters);
    ;//arj_user_msg((FMSG *)misc_buf);
    }
   }
   arj_user_msg((FMSG *)lf);
   ...
   if(cmd==ARJ_CMD_LIST&&debug_enabled&&strchr(debug_opt, 'l')!=NULL)
    msg_printf(M_ENCRYPT_VALUE, ext_hdr_flags);
  }
  if(multivolume_option)
  ...
  if(modify_command)
  {
   ...
   if(security_state)
   {
    arj_user_msg(M_CANT_UPDATE_SEC);
    arj_user_msg((FMSG *)lf);
    file_close(aistream);
    aistream=NULL;
    errors++;
   ...
   if(garble_enabled)
   {
    encryption_version=0;//;garble_init(0);
   ...
   if(cmd==ARJ_CMD_COPY)
   {
    ...
    if(use_ansi_cp==ANSICP_SKIP)
    {
     ...
     {
      arj_user_msg(M_NOT_OEM_CP_ARCHIVE);
      file_close(aistream);
      aistream=NULL;
      errors++;
      errorlevel=ARJ_ERL_WARNING;
  ...
  else
  {
   ...
   if(security_state&&arjsec_opt!=ARJSECP_SKIP)
   {
    ...
    {
     msg_sprintf(misc_buf, M_CANT_HANDLE_ARJSEC_V, method);
     arj_user_msg((FMSG *)misc_buf);
    }
    else
    {
     arj_user_msg(M_VERIFYING_ARJSEC);
     if(arjsec_signature==NULL)
     ...
     {
      delay(5);
      arj_user_msg(M_DAMAGED_SEC_ARCHIVE);
      fclose(aistream);
      aistream=NULL;
      errors++;
      return(0);
     }
     arj_user_msg(M_VALID_ENVELOPE);
   ...
   {
    msg_fprintf(new_stdout, M_EXTRACTING_CMT_TO_F, archive_cmt_name);
 ...
 /* */ /* Nag screen removed */ /* */
  ...
  if(!first_vol_passed)
  {
      ...
      (file_garbled&&!garble_enabled&&(cmd_verb==ARJ_CMD_EXTRACT||cmd_verb==ARJ_CMD_TEST)))
   {
    ...
    msg_fprintf(new_stdout, M_ENTER_PWD);
//    read_line_noecho(tmp_ptr, INPUT_LENGTH);
  ...
  logo_shown=1;
  timestamp_to_str(timetext, &ftime_stamp);
  msg_printf(M_ARCHIVE_CREATED, timetext);
  if(show_ansi_comments)
   printf(cmt_ptr);
  else
   display_comment(cmt_ptr);
  ...
  {
   msg_printf(M_CONTINUE_EXTRACTION);
   if(!query_action())
 ...
 #endif
 {
  ...
  if(target_dir[0]!='\0')
  {
   ...
   #else
    msg_printf(M_QUERY_DEST_DIR, target_dir);
    query_result=query_action();
   #endif
  }
  if(!query_result)
  {
   msg_printf(M_ENTER_INSTALL_DIR);
//   read_line(target_dir, FILENAME_MAX-2);
 ...
 #endif
 {
  ...
  ratio=calc_percentage(total_compressed, total_uncompressed);
  #if SFX_LEVEL>=ARJSFXV
   msg_sprintf(misc_buf, M_TOTAL_STATS, total_files, total_uncompressed, total_compressed, ratio/1000, ratio%1000, msg_ptr, nullstr);
   arj_user_msg((FMSG *)misc_buf);
   if(av_total_files>total_files)
   {
    msg_sprintf(misc_buf, M_TOTAL_STATS, av_total_files, av_uncompressed, av_compressed, ratio/1000, ratio%1000, nullstr, nullstr);
    arj_user_msg((FMSG *)misc_buf);
   }
  #else
   msg_printf(M_TOTAL_STATS, total_files, total_uncompressed, total_compressed, ratio/1000, ratio%1000);
  #endif
  #if SFX_LEVEL>=ARJSFXV
   if(chk_free_space)
   {
 ...
 #endif
 if(valid_envelope)
  msg_printf(M_VALID_ARJSEC, arjsec_signature);
 #if SFX_LEVEL<=ARJSFX
  /* ARJDISP cleanup */
  if(arjdisp_enabled)
  {
 ...
 is_removable=file_is_removable(archive_name);
  ...
  if(mv_cmd_state!=MVC_NONE)
  {
   if(mv_cmd[0]!='\0')
   {
    ...
    {
     msg_printf(M_COMMAND);
     msg_printf(M_FILENAME_FORM, mv_cmd);
    #if TARGET==UNIX
     nputlf();
    #endif
     exec_cmd(mv_cmd);
