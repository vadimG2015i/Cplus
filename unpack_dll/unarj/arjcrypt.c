/*
 * $Id: arjcrypt.c,v 1.1.1.1 2002/03/28 00:01:13 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static unsigned long gost64_key[8]={0L};/* ARJCRYPT v 2.0 encryption key */
...
static void gost_encode(unsigned char FAR *src, unsigned char FAR *dest, int len)
{
 ...
 else
 {
  ...
  while(len--!=0)
  {
   if(last_bytes==0)
    gost_loop(back_code, back_code, gost_key);
   bc_offset[last_bytes]=*(dest++)=*(src++)^bc_offset[last_bytes];
   last_bytes++;
   last_bytes%=8;
  }
 }
 ...
 else
 {
  ...
  while(len--!=0)
  {
   if(last_bytes==0)
    gost_loop(back_code, back_code, gost_key);
   dec_sym=*(src++);
   *(dest++)=dec_sym^bc_offset[last_bytes];
   bc_offset[last_bytes++]=dec_sym;
   last_bytes%=8;
  }
 }
}
...
static void gost_crtkey(unsigned long *seed)
{
 ...
 {
  memcpy(gost_key, gost64_key+sizeof(gost_key), sizeof(gost_key));
  for(i=0; i<KEYGEN_ITERATIONS; i++)
   gost_encode((unsigned char FAR *)tmp_key, (unsigned char FAR *)tmp_key, sizeof(tmp_key));
  memcpy(gost_key, tmp_key, sizeof(gost_key));
 }
...
#endif
{
 ...
 switch(exblock_ptr->mode)
 {
  ...
  case ARJCRYPT_V2_INIT:
   ...
   memset(gost_key, 0, sizeof(gost_key));
   far_strncpy((char FAR *)gost_key, exblock_ptr->password, sizeof(gost_key));
   memset(gost_key, 0, sizeof(gost64_key));
  ...
  case ARJCRYPT_ENCODE:
   gost_encode(exblock_ptr->data, exblock_ptr->data, exblock_ptr->len);
   exblock_ptr->rc=ARJCRYPT_RC_OK;
   break;
  case ARJCRYPT_DECODE:
   gost_decode(exblock_ptr->data, exblock_ptr->data, exblock_ptr->len);
   exblock_ptr->rc=ARJCRYPT_RC_OK;
   break;
  case ARJCRYPT_CIPHER:
  case ARJCRYPT_DECIPHER:
   gost_cipher_proc(exblock_ptr->data, exblock_ptr->data, exblock_ptr->len);
   exblock_ptr->rc=ARJCRYPT_RC_OK;
   break;
  default:
