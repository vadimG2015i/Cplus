/*
 * $Id: environ.c,v 1.2 2002/04/14 22:16:55 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
#if COMPILER==BCC
 #include <dir.h>
#elif COMPILER==MSC||COMPILER==ICC
 #include <direct.h>
#endif
...
#if COMPILER==MSC||COMPILER==ICC||COMPILER==HIGHC||defined(__EMX__)||(TARGET==OS2&&defined(LIBC))
 #include <sys/types.h>
 #include <sys/stat.h>
...
/* An OS/2 macro to get timestamps */
...
/* Attribute comparision for UNIX */
...
/* Returns the number of bytes available in the far heap (quite slow) */
...
long farcoreleft()
{
 /*#if TARGET==DOS
  ...
  #endif
 #else
  return(1048576L);                     /* BUGBUG: TODO: FIXME 
 #endif*/
	 return(1048576L); 
}
#endif
...
//  char *res=new char;
  ...
  _getsystime(&dts);
  ts->ti_hour=dts.tm_hour;
  ts->ti_min=dts.tm_min;
  ts->ti_sec=dts.tm_sec;
  ts->ti_hund=dts.tm_sec/100;
 #elif TARGET==OS2
  DATETIME dts;
...
{
 #if TARGET==OS2
  DosSetPrty(PRTYS_THREAD, priority->class1, priority->delta, 0);
 #else
  #if defined(HAVE_SETPRIORITY)
   setpriority(PRIO_PROCESS, 0, 21-priority->class1);
  #else
...
void check_dir(CHAR * f)        // checks/creates path of file
{
   CHAR *cp,
        d[FILENAME_MAX];
   INT  i;
...
{
	int ret;
	return ret=mkdir((const char*) longname);
 /*union REGS regs;
 struct SREGS sregs;
 char FAR *lnptr;
...
{
	int ret;
 return ret=rmdir((const char*) longname);
 /*union REGS regs;
 struct SREGS sregs;
 char FAR *lnptr;
...
{
	int ret;
 return ret=remove((const char*) longname);
 /*union REGS regs;
 struct SREGS sregs;
 char FAR *lnptr;
...
static int w95_rename(char *longname1, char *longname2)
{
	...
	int ret;
	return ret= rename((const char*) longname1,(const char*) longname2);
/*	union REGS regs;
 struct SREGS sregs;
...
{
	int ret;
  return ret=	_chmod((const char*) longname,pmode);
 /*union REGS regs;
 struct SREGS sregs;
 char FAR *lnptr;
...
unsigned long MakeTime(CONST FILETIME *lpFileTime) // file time
  {
    
	//WORD *lpFatDate=(LPWORD)malloc(sizeof(WORD));
	//WORD *lpFatTime=(LPWORD)malloc(sizeof(WORD)) ;
     long 	result;
	LPSYSTEMTIME lpSystemTimeUTC;
	LPSYSTEMTIME lpSystemTime;
	struct tm * time_ret;
	time_ret=malloc(sizeof(struct tm));
	lpSystemTime=malloc(sizeof(SYSTEMTIME));
	lpSystemTimeUTC=malloc(sizeof(SYSTEMTIME));
 

  if (FileTimeToSystemTime( lpFileTime,  // file time to convert
    lpSystemTimeUTC )  )
  /*if (FileTimeToDosDateTime(lpFileTime, // file time
   lpFatDate,           // MS-DOS date
   lpFatTime    ))*/
  {
    SystemTimeToTzSpecificLocalTime(NULL, lpSystemTimeUTC,lpSystemTime);
    time_ret->tm_sec=lpSystemTime->wSecond;     /* seconds after the minute - [0,59] */
        time_ret->tm_min=lpSystemTime->wMinute;     /* minutes after the hour - [0,59] */
        time_ret->tm_hour=lpSystemTime->wHour;    /* hours since midnight - [0,23] */
        time_ret->tm_mday=lpSystemTime->wDay;    /* day of the month - [1,31] */
        time_ret->tm_mon=lpSystemTime->wMonth-1;     /* months since January - [0,11] */
        time_ret->tm_year=lpSystemTime->wYear-1900;    /* years since 1900 */
        time_ret->tm_wday=lpSystemTime->wDayOfWeek;    /* days since Sunday - [0,6] */
        time_ret->tm_yday=(lpSystemTime->wMonth-1)*30 +lpSystemTime->wDay;    /* days since January 1 - [0,365] */
        time_ret->tm_isdst=1;  
   ...
   /* WORD wMonth; 
    ...
    WORD wSecond; 
    WORD wMilliseconds;
  result=((unsigned long)*(USHORT *)lpFatDate<<16)+(unsigned long)*(USHORT *)lpFatTime;
  return result;
  free(lpFatDate);
  free(lpFatTime);*/
  result=mktime(time_ret);
  ;
  free(time_ret);
  free(lpSystemTime);
  free(lpSystemTimeUTC);
	return result;
  };
  return 0;
...
int w95_findfirst(char *path, struct new_ffblk *new_ffblk, int attrib)
{
  ...
  char * token;
  HANDLE HandleFirst;
  LPWIN32_FIND_DATA lpFindFileData=(LPWIN32_FIND_DATA)malloc(sizeof(WIN32_FIND_DATA)) ;
  pdest = strstr(path,".ARJ");
  if (!pdest) pdest = strstr(path,".arj");
  if (!pdest) pdest = strstr(path,".EXE");
  if (!pdest) pdest = strstr(path,".exe");
  if (pdest)
  {
	  result = (int)(pdest - path);


  filename=(char*)malloc(strlen(path));
  strncpy(filename,path,result);
  ...
  //token=strtok(filename,".ARJ");
  //strcat(filename+pos,".*\0");

  HandleFirst=FindFirstFile(filename,               // file name
	  lpFindFileData );

  new_ffblk->ff_handle=HandleFirst;
 ...
 //{
    strcpy(new_ffblk->ff_name,lpFindFileData->cFileName);
	 
//	FileTimeToSystemTime(&lpFindFileData->ftLastAccessTime,time_s);
	new_ffblk->ff_atime=MakeTime(&lpFindFileData->ftLastAccessTime);//.dwLowDateTime+(MAXDWORD+1)*lpFindFileData->ftLastAccessTime.dwHighDateTime ;
	//time_tm=mktime(&new_ffblk->ff_atime);
	TIMENOW=ctime( &new_ffblk->ff_atime);
	new_ffblk->ff_ctime=MakeTime(&lpFindFileData->ftCreationTime);//.dwLowDateTime+(MAXDWORD+1)*lpFindFileData->ftCreationTime.dwHighDateTime ;
		TIMENOW=ctime( &new_ffblk->ff_ctime);
	new_ffblk->ff_ftime=MakeTime(&lpFindFileData->ftLastWriteTime);//.dwLowDateTime+(MAXDWORD+1)*lpFindFileData->ftLastWriteTime.dwHighDateTime ;
	new_ffblk->ff_attrib=lpFindFileData->dwFileAttributes;
		TIMENOW=ctime( &new_ffblk->ff_ftime);
	new_ffblk->ff_fsize=lpFindFileData->nFileSizeLow+(MAXDWORD+1)*lpFindFileData->nFileSizeHigh;
...
{
	LPWIN32_FIND_DATA lpFindFileData=(LPWIN32_FIND_DATA)malloc(sizeof(WIN32_FIND_DATA)) ;
	//HANDLE HandleAr=(HANDLE)new_ffblk->ff_handle;
	//strcpy(lpFindFileData->cFileName,new_ffblk->ff_name);
    if (FindNextFile(new_ffblk->ff_handle,lpFindFileData))
	{
...
{
	FindClose((HANDLE) new_ffblk->ff_handle);  
...
{
 int ret;
 return ret=	fopen((const char*) longname,"w+" );
 /*union REGS regs;
 struct SREGS sregs;
 char FAR *lnptr;
...
static void finddata_to_properties(struct file_properties *properties, struct new_ffblk *new_ffblk)
{
 ...
 properties->attrib=(ATTRIB)new_ffblk->ff_attrib;
  ...
  /* Hardlink data */
  properties->hl_search=new_ffblk->hl_search;
  properties->hl_search.ref=0;
  properties->islink=0;
 #endif
}
#endif
...
{
 #if TARGET==DOS&&!defined(REGISTER)
  if(lfn_supported!=LFN_NOT_SUPPORTED)
...
static int roll_dirent(struct new_ffblk *new_ffblk)
{
 ...
 while((ent=readdir(new_ffblk->ff_handle))!=NULL)
 {
  ...
  if(m&&!fnmatch(new_ffblk->wildcard, ent->d_name, 0))
  {
       ...
       (attrs&FATTR_DT_UXSPECIAL)))
   {
    ...
    new_ffblk->hl_search.dev=st.st_dev;
    new_ffblk->hl_search.inode=st.st_ino;
    new_ffblk->hl_search.refcount=S_ISDIR(st.st_mode)?1:st.st_nlink;
    break;
   }
  }
 }
...
int lfn_findfirst(char *path, struct new_ffblk *new_ffblk, int attrib)
{
  ...
  if(lfn_supported==LFN_SUPPORTED)
   ...
   #elif COMPILER==MSC
    return(0/*_dos_findfirst(path, attrib, (struct find_t *)new_ffblk)*/);
   #endif
 #elif TARGET==OS2
...
int lfn_findnext(struct new_ffblk *new_ffblk)
{
  ...
  if(lfn_supported==LFN_SUPPORTED)
   ...
   #elif COMPILER==MSC
    return(0/*_dos_findnext((struct find_t *)new_ffblk)*/);
   #endif
 #elif TARGET==OS2
  HDIR handle;
  #ifdef __32BIT__
...
{
	DWORD access;
  if(access=GetFileAttributes(name)==FILE_ATTRIBUTE_READONLY)
return 0;
return -1;   
 /*#if TARGET==DOS
...
void delay(unsigned int seconds)
{
  ...
  #if COMPILER==BCC
   sleep(seconds);
  #elif COMPILER==MSC                   /* Inappropriate way for TANDY 2000! */
...
void mem_stats()
{
 ...
 {
  #if SFX_LEVEL>=ARJ
   msg_fprintf(new_stdout, M_MEMSTATS, coreleft(), verify_heap(), farcoreleft(), verify_far_heap(), get_stack_space());
  #else
   msg_fprintf(new_stdout, M_MEMSTATS, coreleft(), farcoreleft());
  #endif*/
...
{
    FILE * res=NULL;
	if (res=_fsopen(name,"r",_SH_DENYNO)) 
	{ ;
...
{  FILE * res;
  ...
  else
  {
   ...
   #endif
   #if TARGET==DOS
    if(1)                     // Sharing modes for DOS v 3.0+ 
    {
     #if COMPILER==BCC
      if(tmp_mode[1]=='+'||tmp_mode[1]!='\0'&&tmp_mode[2]=='+')
       oflag|=O_DENYWRITE;
...
unsigned long file_getfree(char *name)
{
  ...
  #elif COMPILER==MSC
   if(1/*_dos_getdiskfree((unsigned int)drive, &dtable)*/)
    return(ULONG_MAX);
   else
   {
...
int read_line(char *buf, int size)
{
 /*#if TARGET==DOS
  ...
  return(chars_read);*/
/*#ifdef (TARGET==OS2&&(COMPILER==MSC||defined(LIBC)))||TARGET==UNIX
  int chars_read;
  int cchar;
...
int match_wildcard(char *name, char *wcard)
{
  ...
  {
   cchar=wcard[0];
   if(cchar=='*') goto l_14af;
   if(cchar=='.') goto l_14c8;
   if(cchar=='?') goto l_149e;
   if(toupper(name[0])!=toupper(wcard[0]))
    return(0);
   name++;
   wcard++;
   continue;
 l_149e: if(name[0]=='\0'||name[0]=='.')
          goto l_14ee;
         else
          goto l_14ed;
 l_14ae: name++;
 l_14af: if(name[0]=='\0') goto l_14bc;
         if(name[0]!='.') goto l_14ae;
         goto l_14bc;
 l_14bb: wcard++;
 l_14bc: if(wcard[0]=='\0')
          goto l_14ef;
 ...
 l_14c8: if(name[0]=='\0')
          goto l_14ee;
 l_14cf: if(toupper(name[0])!=toupper(wcard[0]))
          return(0);
 l_14ed: name++;
 l_14ee: wcard++;
 l_14ef:;
...
int reset_drive(char *name)
{
  ...
  if(name[1]==':')
   ...
   drive=getdisk()+1;
  if(0)                       /* DOS 2.x - A: and B: are removable */
  {
   //bdos(0x0D, 0, 0);
   return(1);
  }
...
int file_setftime(char *name, unsigned long ftime)
{
  ...
  #elif COMPILER==MSC
  result=0;//_dos_setftime(fileno(stream), *((unsigned int *)&ftime+1), *(unsigned int *)&ftime);
  #endif
  fclose(stream);
  return(result);
 #elif TARGET==OS2
  ...
  if((access_status=file_chmod(tmp_path, 0, 0))==-1)
  {
   if(!no_queries)
   {
     ...
     if((no_queries=query_action(0, QUERY_CREATE_DIR, misc_buf))==0)
      return(1);
    #else
     msg_printf(M_QUERY_CREATE_DIR, path);
   ...
   if(file_mkdir(tmp_path)&&errno!=ENOENT)
   {
    ...
    #if SFX_LEVEL>=ARJSFXV
     msg_fprintf(new_stdout, M_CANT_MKDIR, tmp_path);
    #else
     msg_printf(M_CANT_MKDIR, tmp_path);
    #endif
    #if SFX_LEVEL>=ARJ
     error_freport();
    #endif
...
/* Copies one file to another, performing a check if needed. Issues a native
   API if the target platform supports it. */
...
int file_copy(char *dest, char *src, int chk)
{
  ...
  switch(rc)
  {
   ...
   case ERROR_SHARING_VIOLATION:
    msg_printf(M_CANTOPEN, src);
    break;
   case ERROR_DISK_FULL:
    msg_printf(M_DISK_FULL);
    break;
   default:
    msg_printf(M_CRC_ERROR);
    break;
  }
  if(rc)
  {
  ...
  if(bytes_read==0&&chk)
  {
   ...
   {
    if(fread(dbuf, 1, PROC_BLOCK_SIZE/2, istream)!=bytes_read)
     break;
   ...
   fclose(ostream);
   fclose(istream);
   fprintf(new_stdout, vd_space);
   msg_printf(bytes_read==0?M_OK:M_CRC_ERROR);
  }
...
#endif
{
 ...
 struct new_ffblk new_ffblk;            /* OS-dependent data block */
 int result;                            /* findfirst/findnext() result */
 #ifdef REARJ
  char tmp_name[CCHMAXPATH+20];         /* +20 to make "\*.*" appendable */
  char subdir_spec[CCHMAXPATH+20];
  char subdir_wildcard[20];
 #else
  struct file_properties properties;    /* Universal block */
  char *tmp_name;
  char *subdir_wildcard;                /* Subdirectory entries */
  char *subdir_spec;                    /* Subdirectory name */
  int pathspec_len;                     /* Maximum path length */
 #endif

 #ifndef REARJ
  subdir_wildcard=NULL;
  subdir_spec=NULL;
  ...
  if((tmp_name=(char *)malloc(pathspec_len+50))==NULL)
   goto l_error;
  if((subdir_wildcard=(char *)malloc(pathspec_len+50))==NULL)
   goto l_error;
 #endif
 flush_kbd();
 if(!expand_wildcards)
 ...
 else
 {
  ...
  if(recurse_subdirs)
  {
   ...
   result=lfn_findfirst(subdir_spec, &new_ffblk, attr_mask);
   while(result==0)
   {
    ...
    if(new_ffblk.ff_attrib&STD_DIR_ATTR&&strcmp(new_ffblk.ff_name, cur_dir_spec)&&strcmp(new_ffblk.ff_name, up_dir_spec))
    {
     ...
     if(strlen(tmp_name)+strlen(new_ffblk.ff_name)+strlen(subdir_wildcard)+2>=CCHMAXPATHCOMP)
     {
      #ifdef REARJ
       msg_printf(M_MAXPATH_EXCEEDED, CCHMAXPATH, tmp_name);
      #else
       msg_fprintf(new_stdout, M_MAXPATH_EXCEEDED, CCHMAXPATH, tmp_name);
      #endif
     }
     else
     {
  ...
  while(result==0)
  {
   ...
   if(!(new_ffblk.ff_attrib&STD_DIR_ATTR)||(strcmp(new_ffblk.ff_name, cur_dir_spec)&&strcmp(new_ffblk.ff_name, up_dir_spec)))
   {
    ...
    /* ASR fix for 2.76.04 */
    if(strlen(tmp_name)+strlen(new_ffblk.ff_name)>=CCHMAXPATH)
    {
     #ifdef REARJ
      msg_printf(M_MAXPATH_EXCEEDED, CCHMAXPATH, new_ffblk.ff_name);
     #else
      msg_fprintf(new_stdout, M_MAXPATH_EXCEEDED, CCHMAXPATH, new_ffblk.ff_name);
     #endif
    }
    else
    {
  ...
  lfn_findclose(&new_ffblk);            /* BUG: Original REARJ v 2.28 didn't
