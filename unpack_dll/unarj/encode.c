/*
 * $Id: encode.c,v 1.1.1.1 2002/03/28 00:02:24 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
void send_block()
{
 ...
 if(!unpackable)
 {
  root=make_tree(NCC, c_freq, c_len, c_code);
  size=c_freq[root];
  putbits(16, size);
  if(root>=NCC)
  {
...
void allocate_memory()
{
 ...
 if((c_freq=calloc(NCC*2-1, sizeof(*c_freq)))==NULL)
  error(M_OUT_OF_NEAR_MEMORY);
 if((c_code=farcalloc(NCC, sizeof(*c_code)))==NULL)
  error(M_OUT_OF_MEMORY);
 if((heap=farcalloc(NCC+1, sizeof(*heap)))==NULL)
  error(M_OUT_OF_MEMORY);
 for(i=0; i<NP; i++)
  p_freq[i]=0;
 depth=0;
...
static void NEAR huf_encode()
{
 ...
 unsigned int fetch;
 unsigned int max_fetch;                /* For comparision */
 short FAR *fptr;
 short FAR *dtptr;
 unsigned char *tptr;
...
void encode(int method)
{
 ...
 if(debug_enabled)
 {
  ...
  if(strchr(debug_opt, 'v')!=NULL)
   msg_fprintf(new_stdout, M_PRECOMP_STAT, numchars, dicbit, dicsiz_cur, dic_alloc, treesize);
 }
