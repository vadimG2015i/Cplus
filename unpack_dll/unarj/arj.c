/*
 * $Id: arj.c,v 1.3 2002/07/04 22:27:10 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static void file_arg_cleanup(struct flist_root *flist)
{
 ...
 for(cur_arg=0; cur_arg<file_args; cur_arg++)
 {
  ...
  {
   msg_fprintf(new_stdout, M_CANT_FIND, f_arg_array[cur_arg]);
 ...
 if(flist->files>0)
 {
  ...
  for(cur_file=0; cur_file<flist->files; cur_file++)
  {
   ...
   msg_fprintf(new_stdout, M_CANT_FIND, tmp_name);
   if(listfile_err_opt)
   {
    if(errorlevel==ARJ_ERL_SUCCESS)
     errorlevel=ARJ_ERL_WARNING;
...
static int  cmd_setup(int *cmd, int *is_add_cmd)
{
 int cnv_cmd;
 int add_cmd;
 char *tmp_swptr;
...
static int fetch_extension(int offset, char *dest)
{
 ...
 do
  offset++;
 while(archive_ext_list[offset]!='\0'&&archive_ext_list[offset]!='.');
...
static int parse_cmdline(char *token, int cmd)
{
 ...
 if(debug_enabled&&strchr(debug_opt, 'v')!=NULL)
  msg_fprintf(new_stdout, M_TOKEN, token);
 if(is_switch(token))
  analyze_arg(token);
 else
 {
  if(cmd==0)
  {
   ...
   {
    msg_fprintf(new_stdout, M_INVALID_COMMAND, token);
...
static void ctrlc_handler(SIGHDLPARAMS)
{
 ...
 else
 {
  ...
  msg_fprintf(new_stdout, M_BREAK_SIGNALED);
  return(ARJ_ERL_BREAK);
 }
}
...
static void term_handler(SIGHDLPARAMS)
{
 error_occured=1;                       /* ARJ needs termination */
 signal(SIGTERM, NULL);
 msg_fprintf(new_stdout, M_SIGTERM);
 return(ARJ_ERL_BREAK);
}
#endif
...
static void final_cleanup(void)
{
 ...
 if(debug_enabled&&strchr(debug_opt, 'v')!=NULL)
 {
  msg_printf(M_EXITING_PROGRAM);
  if(double_shutdown)
   msg_printf(M_HERE_TWICE);
  if(verify_heap())
   msg_printf(M_BAD_HEAP);
 }
 if(double_shutdown)
  return;
 double_shutdown=1;
 ...
 #endif
 #if TARGET==UNIX
  if(hl_entries.list!=NULL)
   farfree(hl_entries.list);
 #endif
 ...
 if(debug_enabled&&strchr(debug_opt, 't')!=NULL)
 {
  ...
  msg_fprintf(new_stdout, M_FINAL_TIMING, ticks);
 }
 if(!store_by_suffix)
...
// strcpy(test,test1);//by va
 ...
 //if(quiet_mode==ARJ_QUIET||quiet_mode==ARJ_QUIET1)
...
// check_fmsg(CHKMSG_SKIP);
 if((tmp_stdout=new_stdout)==new_stderr&&!is_registered)
  new_stdout=stderr;
 msg_strcpy(strcpy_buf, M_VERSION);
 //msg_sprintf(misc_buf, M_ARJ_BANNER, M_ARJ_BINDING, strcpy_buf, build_date);
 //scroll_puts((FMSG *)misc_buf);
 if(!is_registered&&!msg_strcmp((FMSG *)(regdata+REG_KEY2_SHIFT), M_REG_TYPE))
 {
  msg_sprintf(misc_buf, M_REGISTERED_TO, regdata+REG_NAME_SHIFT);
  scroll_puts((FMSG *)misc_buf);
 }
 ...
 if(rsp_name[0]=='\0')
 {
  ...
  {
   /* In ARJ v 2.72 there is a malloc_str() here for the reason if
      fix_ansi_name() changes argv[], but we omit it to conserve RAM */
/*   if(!ansi_cpf)
    fix_ansi_name(argv[cur_arg]); */    /* Removed -- ASR fix to v 2.72 */
 ...
 else
 {
  ...
  while(fgets(header, FILENAME_MAX, stream)!=NULL)
  {
   ...
   endptr=tptr;
   tptr=tmp_ptr;
   while((unsigned int)tptr<(unsigned int)endptr)
   {
    /* ASR fix: check for overrun -- 25/08/2001 */
    while(*tptr=='\0'&&(unsigned int)tptr<(unsigned int)endptr)
     tptr++;
    if((unsigned int)tptr<(unsigned int)endptr)
    {
 ...
 if(cmd==ARJ_CMD_RECOVER)
 {
  ...
  msg_printf(M_REPAIRED_FILE, tmp_tmp_filename);
  tmp_tmp_filename[0]='\0';
  free(name);
  return(ARJ_ERL_SUCCESS);
 }
...
//  check_fmsg(CHKMSG_NOSKIP);
 ...
 #if TARGET==DOS
  if(lfn_supported!=LFN_NOT_SUPPORTED)
   scroll_puts(M_LFN_ENABLED);
 #elif TARGET==OS2
  if(ea_supported)
   scroll_puts(M_EA_ENABLED);
 #endif
...
//   read_line_noecho(tptr, INPUT_LENGTH);
   ...
   if(is_add_cmd||cmd=='G')
   {
    msg_fprintf(new_stdout, M_VERIFY_PWD);
//    read_line_noecho(tptr, INPUT_LENGTH);
 ...
 {
  msg_fprintf(new_stdout, M_CANT_FIND, archive_name);
  errorlevel=ARJ_ERL_CANTOPEN;
  errors++;
 }
 if(debug_enabled&&strchr(debug_opt, 't')!=NULL)
  msg_fprintf(new_stdout, M_N_TICKS, get_ticks()-ticks);
// check_fmsg(CHKMSG_SKIP);
 if(quiet_mode==ARJ_QUIET2)
  new_stdout=new_stderr;
 
 target_dir=malloc(strlen(Directory));
  strcpy(target_dir,Directory);
