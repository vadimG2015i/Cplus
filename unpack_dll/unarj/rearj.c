/*
 * $Id: rearj.c,v 1.1.1.1 2002/03/28 00:03:19 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
#define LARGE_FLIST_SIZE       20000    /* For "*.*"-alike processing */
#define EXCL_FLIST_SIZE        10000    /* For "*.*"-alike processing */
...
/* Copies one file to another with mandatory comparision */
...
static void unlink_all(char *path)
{
 ...
 #endif
 if(no_file_activity)
  msg_printf(M_DELETING, tmp_path);
...
static int truename(char *dest, char *name)
{
    ...
    dest[0]!='\0')
 {
  if(file_chdir(dest))
  {
   msg_printf(M_PATH_NOT_FOUND, dest);
termination:
...
static int convert_archive(char *name)
{
 ...
 {
  msg_printf(M_SKIP_CANT_FIND, name);
 ...
 if(src_type==-1)
 {
  msg_printf(M_SKIP_UNKNOWN_TYPE, full_name);
 ...
 {
  msg_printf(M_SKIP_TGT_EXISTS, full_name);
  return(REARJ_ERL_TGT_EXISTS);
 }
 if(!update_with_backups&&repack)
 {
  msg_printf(M_SKIP_REPACK, full_name);
  return(REARJ_ERL_UPD_SKIPPED);
 }
 msg_printf(M_CONVERTING_ARCHIVE, full_name, archivers[src_type].suffix, archivers[target_type].suffix);
 if(conversion_query)
 {
  msg_printf(M_QUERY_CONVERT);
  if(!query_action())
  {
   msg_printf(M_SKIPPING, full_name);
   return(REARJ_ERL_UPD_SKIPPED);
  }
 }
 ...
 if(run_preunpack_cmd)
 {
  ...
  msg_printf(M_EXECUTING_PRE, cmd_buffer);
  exec_cmd(cmd_buffer);
 }
 sprintf(cmd_buffer, archivers[src_type].unpack_cmd, full_name);
 msg_printf(M_EXECUTING, cmd_buffer);
 exec_rc=exec_exe(cmd_buffer);
 if(exec_rc!=0)
 {
  if(exec_rc==-1)
   msg_printf(M_SKIP_EXE_MISSING, full_name);
  else
   msg_printf(M_SKIP_UNPACK_ERR, full_name, exec_rc);
  return(REARJ_ERL_UNPACK);
 }
 if(!archivers[target_type].subdirs_supported&&check_for_dirs())
 {
  msg_printf(M_SKIP_UNSUPP_DIR, full_name);
  return(REARJ_ERL_DIRECTORIES);
 }
 if(!check_for_entries())
 {
  msg_printf(M_SKIP_NO_FILES, full_name);
  return(REARJ_ERL_UNPACK);
 }
 if(run_precount_cmd)
 {
  sprintf(cmd_buffer, "%s %s", precount_cmd_text, full_name);
  msg_printf(M_EXECUTING_CNT, cmd_buffer);
  exec_cmd(cmd_buffer);
 }
 if(!skip_size_check)
 ...
 if(run_extract_cmd)
 {
  msg_printf(M_EXECUTING_EXTR, extract_cmd_text);
  exec_rc=exec_exe(extract_cmd_text);
  if(exec_rc!=0)
  {
   if(exec_rc==-1)
    msg_printf(M_SKIP_V_EXE_MISSING, full_name);
   else
    msg_printf(M_SKIP_V_ERR, full_name, exec_rc);
 ...
 if(convert_nested_archives)
 {
  ...
  {
   if(exec_rc==-1)
    msg_printf(M_SKIP_REARJ_MISSING, full_name);
   else
    msg_printf(M_SKIP_REARJ_FAILED, full_name, exec_rc);
 ...
 if(update_with_backups&&repack)
 {
  ...
  if(!no_file_activity)
   ...
   {
    msg_printf(M_SKIP_CANT_RENAME, full_name, bak_name);
    return(REARJ_ERL_RENAME);
   }
  if(!no_file_activity)
   msg_printf(M_BACKED_UP, full_name, bak_name);
 }
 ...
 if(!skip_size_check)
 {
  msg_printf(M_VERIFYING_SIZE, old_count, old_size);
  unlink_all(tmp_dir);
  if(file_chdir(tmp_dir))
   error(M_CANT_CHDIR, tmp_dir);
  ...
  msg_printf(M_FOUND_SIZE, new_count, new_size);
  if(old_count!=new_count)
  {
   msg_printf(M_SKIP_COUNT_MISMATCH, full_name);
   if(!no_file_activity)
    if(file_unlink(tmp_name))
     error(M_CANT_UNLINK, tmp_name);
   if(update_with_backups&&repack&&!no_file_activity)
  ...
  {
   msg_printf(M_SKIP_SIZE_MISMATCH, full_name);
   if(!no_file_activity)
    if(file_unlink(tmp_name))
     error(M_CANT_UNLINK, tmp_name);
   if(update_with_backups&&repack&&!no_file_activity)
 ...
 if(cnv_diskette_archives)
 {
  ...
  {
   msg_printf(M_SKIP_DISK_FULL, full_name);
   if(!no_file_activity)
    if(file_unlink(tmp_name))
     error(M_CANT_UNLINK, tmp_name);
   if(update_with_backups&&repack&&!no_file_activity)
 ...
 if(delete_original_archives)
 {
  if(!repack)
  {
   msg_printf(M_DELETING_2, full_name);
   if(!no_file_activity)
    if(file_unlink(full_name))
     error(M_CANT_UNLINK, full_name);
  }
  ...
  {
   msg_printf(M_DELETING_2, bak_name);
   if(!no_file_activity)
    if(file_unlink(bak_name))
     error(M_CANT_UNLINK, bak_name);
  }
 ...
 if(testing_mode&&!delete_original_archives)
 {
  if(update_with_backups&&repack)
  {
   if(file_exists(full_name))
    if(file_unlink(full_name))
     error(M_CANT_UNLINK, full_name);
   msg_printf(M_RENAMING, bak_name, full_name);
   if(!no_file_activity)
   {
    if(rename_with_check(bak_name, full_name))
     error(M_CANTRENAME, bak_name, full_name);
  ...
  {
   msg_printf(M_DELETING_2, target_name);
   if(!no_file_activity)
    if(file_unlink(target_name))
     error(M_CANT_UNLINK, target_name);
  }
 ...
 msg_printf(M_OLD_SIZE, old_fsize);
 msg_printf(M_NEW_SIZE, new_fsize);
 msg_printf(M_SAVINGS_SIZE, gain);
 printf(lf);
...
static void analyze_rearj_sw(char *arg)
{
 ...
 else if(sw=='X'&&*swptr!='\0')
 {
  if(!is_registered)
   ...
   wild_list(&flist_exclusion, swptr, 0, 0, 0, NULL);
 }
 else if(is_registered)                 /* Throw all disabled options away */
...
static void parse_rearj_cfg()
{
 ...
 msg_printf(M_USING_CONFIG, cfg_path);
 ...
 else
 {
  ...
  else
  {
   ...
   {
    msg_printf(M_NO_ANTIVIRUS_PATH);
    msg_printf(M_IGNORED_FOR_COMP);
    delay(4);
   }
  }
...
// msg_printf(M_REARJ_BANNER, build_date);
 #ifdef TZ_VAR
  tzset();
 #endif
 build_crc32_table();
 ...
 if(!is_registered&&!msg_strcmp((FMSG *)regdata+REG_KEY2_SHIFT, M_REG_TYPE))
  msg_printf(M_REGISTERED_TO, regdata+REG_NAME_SHIFT);
 limit=20;
 total_suffixes=0;
 ...
 if((env_ptr=getenv(rearj_sw))!=NULL)
 {
  ...
  msg_printf(M_USING_REARJ_SW, env_dup);
 ...
 {
  msg_printf(strform, M_REARJ_COMMANDS);
  msg_printf(strform, M_REARJ_RCODES);
/*  msg_printf(strform, is_registered?M_REARJ_S_COMMANDS:M_REARJ_COMMANDS); */
/*  if(!is_registered)
   msg_printf(strform, M_REARJ_RCODES); */ /* ASR fix (MS C adaptation) */
  exit(REARJ_ERL_SUCCESS);
 }
/* is_registered=!is_registered; */
 is_registered=1;                       /* HACK: enable the comm. features */

 if(!is_registered&&regdata[REG_NAME_SHIFT]!='\0')
  pause_error(M_PROGRAM_CRC_ERROR);
 ...
 {
  msg_printf(internal_archives_only?M_NO_FILES_INT:M_NO_FILES);
 ...
 if(cnv_diskette_archives&&count_files()>0)
  msg_printf(M_CWD_MUST_BE_EMPTY);
 if(tmp_dir==NULL)
 {
  tmp_dir=tmpnam(NULL);
 ...
 for(cur_file=0; cur_file<flist_main.files; cur_file++)
 {
  ...
  if(fetch_keystrokes())
  {
   msg_printf(M_OK_TO_QUIT);
   if(query_action())
   {
    msg_printf(M_QUITTING);
    exit(REARJ_ERL_WARNING);
   }
  }
     ...
     (!pick_older||!ts_valid(ts_older)||ts_cmp(&timestamp, &ts_older)<0))
  {
   ...
   if(file_rmdir(tmp_dir))
    msg_printf(M_CANT_RMDIR, tmp_dir);
   if(file_mkdir(tmp_dir))
    error(M_CANT_MKDIR, tmp_dir);
  }
 }
 ...
 msg_printf(M_TOTAL_SECONDS, time_diff);
 msg_printf(M_TOTAL_CONVERTED, total_files);
 msg_printf(M_TOTAL_SKIPPED, skip_count);
 msg_printf(M_OLD_SIZE, total_old_fsize);
 msg_printf(M_NEW_SIZE, total_new_fsize);
 msg_printf(M_SAVINGS_SIZE, total_gain);
 if(logging_enabled)
 {
  gettime(&log_ts);
 ...
 exit((skip_count>0)?exit_code:REARJ_ERL_SUCCESS);
}
