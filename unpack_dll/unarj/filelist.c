/*
 * $Id: filelist.c,v 1.1.1.1 2002/03/28 00:02:55 andrew_belov Exp $
 * ---------------------------------------------------------------------------
 ...
 * sequentally composed, then sequentally read. No random access.
 *
 */
...
#define FLIST_BLOCK_INCREMENT      8    /* Number of blocks to preallocate
...
#define LIST_END          0x7FFFFFFF    /* Indicates end of enumerator list */
...
void flist_cleanup_proc(struct flist_root *root)
{
 ...
 {
  msg_printf(M_XLIST_BLOCKS, root->table->xlist_blocks);
  if(root==&flist_main)
   msg_printf(M_HASH_MATCHES, hash_matches, crc_matches);
 }
 if(root->storage==BST_FAR)
 {
...
int add_entry(struct flist_root *root, char *name, FILE_COUNT *count, struct file_properties *properties)
{
 ...
 if(root->files>=root->maxfiles)
 {
   ...
   msg_fprintf(new_stdout, M_NAMES_LIMIT, root->maxfiles, name);
  #else
   msg_printf(M_NAMES_LIMIT, root->maxfiles, name);
  #endif
  return(-1);
 }
 ...
 if(debug_enabled&&strchr(debug_opt, '.')!=NULL)
  msg_printf(M_TOKEN, name);
 /* Missing in version 2.72 */
/* if(properties!=NULL&&root==&flist_main)
 {
 ...
 if(idblock_ptr->total_entries+1>ENTRIES_PER_BLOCK||(tmp_offset+(long)extend_len+1>(long)(FLIST_BLOCK_SIZE-sizeof(struct idblock)-2)))
 {
  ...
  /* WARNING: compiler-dependent... */
   ...
   {
    msg_fprintf(new_stdout, M_HASH_MEM_LACK, name);
    return(-1);
   }
   farfree(tmp_ptr);
  #endif
  ...
  root->table->enumerators[tmp_hiblock+1]=LIST_END;
 }
 root->table->not_flushed=1;
  ...
  if(dir_num==0)
  {
   if(root->d_files>=root->d_boundary)
   {
    ...
    if((names_ptr=(char FAR * FAR *)farrealloc(root->d_names, diff*sizeof(char FAR *)))==NULL)
    {
      ...
      msg_fprintf(new_stdout, M_HASH_MEM_LACK, name);
     #else
      msg_printf(M_HASH_MEM_LACK, name);
     #endif
     return(-1);
    }
...
#endif
{
  ...
  if(debug_enabled&&strchr(debug_opt, 'v')!=NULL)
   msg_printf(M_SEARCH_FLAG, type);
  root->table=malloc_msg(sizeof(struct flist_table));
 #if TARGET==DOS
  if(1/*detect_xms()*/&&filelist_storage==BST_XMS)
  {
   root->storage=BST_XMS;
   //get_xms_entry();
   if(0/*!xms_malloc(XMS_BLOCK_PREALLOC*FLIST_BLOCK_SIZE, root)*/)
  ...
  root->table->enumerators[1]=LIST_END;
