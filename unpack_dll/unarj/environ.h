/*
 * $Id: environ.h,v 1.3 2002/12/13 21:51:54 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
#if defined(_UNIX)
 #define TARGET                 UNIX
#elif defined(_OS2)||defined(__OS2__)||defined(__EMX__)
 #define TARGET                  OS2
#else
 #define TARGET                  DOS
#endif
...
#if COMPILER==BCC
 #include <dir.h>
#elif COMPILER==MSC||COMPILER==ICC||COMPILER==HIGHC||COMPILER==WCC
 #include <direct.h>
#endif
...
#elif COMPILER==MSC
 #define farfree              free
 #define farmalloc(s)  malloc((size_t)s)
 #define farrealloc(p, s)  realloc(p, (size_t)s)
 #define farcalloc(n, s)  calloc((size_t)(n), (size_t)(s))
 #define coreleft()            1048576      /* Size of largest unallocated block */
#endif
...
/* Far memory comparision macro/function */
...
#if COMPILER==MSC||COMPILER==ICC||TARGET==UNIX||COMPILER==HIGHC||COMPILER==WCC
 /* LIBCS.DLL complains about missing message resources */
 #if defined(LIBC)&&TARGET==OS2
  #define error_report()
  #define error_freport()
 #else
  #define error_report() \
          { \
           msg_printf(M_ERROR_CODE, errno, strerror(errno)); \
           msg_printf(lf); \
          }
  #define error_freport() \
          { \
           msg_fprintf(new_stdout, M_ERROR_CODE, errno, strerror(errno)); \
           msg_fprintf(new_stdout, lf); \
          }
 #endif
#else
 #define error_report() msg_printf(M_ERROR_CODE, errno, strerror(errno))
 #define error_freport() msg_fprintf(new_stdout, M_ERROR_CODE, errno, strerror(errno))
#endif
...
/* Maximum size of path specification is reset to 260 bytes */

#ifndef CCHMAXPATH
 #define CCHMAXPATH              260
#endif

/* Maximum size of path component (filename or pathname) is 256 bytes */
...
#elif TARGET==UNIX
 #define PARAMS_MAX             1024    /* May be less */
 #define SFLIST_MAX             1024
#else
 #error *** Command-line capacity not defined!
#endif
...
/* FAR memory block size limit */
 ...
 #define FLIST_ALLOC_MAX        65535
#else
 #error *** Far memory allocation limits not defined!
#endif
...
#define PROC_BLOCK_SIZE        27648    /* Size of sequental processing block */
#if TARGET==DOS
...
#if TARGET==UNIX
 #define make_executable(stream) fchmod(fileno(stream), 0755)
 #define make_nonexecutable(stream) fchmod(fileno(stream), 0644)
#else
...
long farcoreleft();
#endif
#if COMPILER==MSC||COMPILER==ICC||COMPILER==HIGHC||COMPILER==WCC
int getdisk();
