/*
 * $Id: arj_arcv.c,v 1.2 2002/04/08 22:07:24 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static unsigned int hget_word()
{
 ...
 result=(int)(*(unsigned short *)tmp_hptr);
...
{
 *(unsigned short *)tmp_hptr=(unsigned short)w;
...
{
 *(unsigned long *)tmp_hptr=(unsigned long)l;
 tmp_hptr+=sizeof(unsigned long);
}
...
static void display_hdr_error_proc(FMSG *errmsg, char *name, unsigned int l)
{
 ...
 if(!ignore_archive_errors)
  error(errmsg, name);
 msg_fprintf(new_stdout, errmsg, name);
 nputlf();
}
...
#endif
{
 unsigned short header_id;
  ...
  char *hdr_filenameOem=(char*)malloc(260);
  char *hdr_filenameAnsi=(char*)malloc(260);
 #endif
 ...
 hdr_filename=&header[first_hdr_size];
  OemToChar(hdr_filename,hdr_filenameOem);
  strcpy(hdr_filename,hdr_filenameOem);
 ...
 while((header_id=fget_word(stream))!=0)
 {
   ...
   else
   {
    ...
    tmp_eh->flags=is_continued?EH_UNPROCESSED:EH_PROCESSING;
    while(remainder>0)
    {
...
static void proc_ext_hdr(unsigned int action)
{
 ...
 {
  if((unsigned long)cur_capacity+2<=p_eh->size)
  {
  ...
  else
  {
   ...
   cur_capacity-=p_eh->size+EXT_HDR_OVERHEAD;
   cont_id=0;
  }
  if(action&EHUF_WRITE)
  {
   ...
   fput_longword(crc32term^CRC_MASK, aostream);
  }
  p_eh=p_eh->next;
 }
...
void write_header()
{
 ...
 fput_longword(header_crc=crc32term^CRC_MASK, aostream);
...
int rename_file()
{
 msg_printf(M_CURRENT_FILENAME, filename);
 msg_printf(M_ENTER_NEW_FILENAME);
...
int supply_comment(char *cmtname, char *name)
{
 ...
 replicate_comment();
 msg_printf(M_CURRENT_COMMENT, name);
 ...
 if(cmtname[0]=='\0')
 {
  ...
  msg_printf(M_ENTER_COMMENT, maxlines, name);
  for(curline=0; curline<maxlines; curline++)
  {
   fprintf(new_stdout, le_prompt, curline+1);
...
{
 #if SFX_LEVEL>=ARJSFXV
  msg_fprintf(new_stdout, M_SKIPPED, filename);
 #else
  msg_printf(M_SKIPPED, filename);
 #endif
 skip_compdata();
}
...
void special_processing(int action, FILE *stream)
{
 ...
 {
  case CFA_UNMARK:
   msg_printf(M_UNMARKING, filename);
  ...
  case CFA_MARK_INCREMENT:
   ...
   break;
  case CFA_REMPATH:
   msg_printf(M_REMOVING_PATH, filename);
   far_strcpyn(comment, (char FAR *)hdr_comment, COMMENT_MAX);
  ...
  case CFA_GARBLE:
   if(!(arj_flags&GARBLED_FLAG))
   {
    msg_printf(M_GARBLING, filename);
    arj_flags|=GARBLED_FLAG;
  ...
  case CFA_UNGARBLE:
   if(arj_flags&GARBLED_FLAG)
   {
    msg_printf(M_UNGARBLING, filename);
    arj_flags&=~GARBLED_FLAG;
    garble_task=2;
    create_header(0);
    total_files++;
...
//  garble_init(password_modifier);
 ...
 while(compsize>0L)
 {
  if(fread(pbuf, 1, count, stream)!=count)
   error(M_CANTREAD);
  if(garble_task==1)
       garble_task=0; //   garble_encode_stub(pbuf, count);
...
static void addition_msg(int is_update, int is_replace, char *filespec)
{
 ...
 {
  if(is_update)
   msg_printf(M_UPDATING);
  else if(is_replace)
   msg_printf(M_REPLACING);
  else
   msg_printf(M_ADDING);
  if(verbose_display==VERBOSE_STD)
  {
   if(file_type==ARJT_BINARY)
    msg_printf(M_BINARY_FILE);
   else if(file_type==ARJT_TEXT)
    msg_printf(M_TEXT_FILE);
   else if(file_type==ARJT_DIR)
    msg_printf(M_DIRECTORY);
   else if(file_type==ARJT_UXSPECIAL)
    msg_printf(M_UXSPECIAL_FILE);
  }
  if(continued_prevvolume&&eh_find_pending(eh)==NULL)
   msg_printf(M_AT_POSITION, format_filename(filespec), resume_position);
  else
   fprintf(new_stdout, strform, format_filename(filespec));
  if(!verbose_display)
  ...
  else
  {
   nputlf();
   if(method==0)
    msg_printf(M_STORING);
   else
    msg_printf(M_COMPRESSING, method);
   msg_printf(M_N_BYTES, uncompsize);
  }
 }
}
...
static int open_input_file()
{
 ...
 if((encstream=file_open(filename, read_perms[file_type%2]))!=NULL)
  return(0);
 error_report();
 msg_printf(M_CANTOPEN, filename);
 nputlf();
 e=1;
 if(no_inarch)
 {
...
int pack_file(int is_update, int is_replace)
{
 ...
 if(is_update&&!skip_ts_check)
 {
  ...
  if(needs_skip)
  {
   if(verbose_display)
   {
    msg_printf(M_NO_CHANGE, format_filename(filename));
   }
 ...
 if(fix_longnames)
 {
  ...
  {
   resolve_longname(ea_res, target_dir);
   res_len=strlen(ea_res);              /* To skip <res_len> bytes later */
  }
 ...
 if(exclude_paths==EP_BASEDIR)
  default_case_path(hdr_filename, (fix_longnames?ea_res:filename)+res_len);
 else if(exclude_paths==EP_PATH)
  split_name(fix_longnames?ea_res:filename, NULL, hdr_filename);
 else
  default_case_path(hdr_filename, fix_longnames?ea_res:filename);
 ...
 if(!continued_prevvolume)
 {
  ...
  if(file_type==ARJT_UXSPECIAL)
  {
   ...
   {
    msg_printf(M_CANT_QUERY_UXSPEC);
    return(0);
   }
  ...
  if(do_chown&&!query_owner(&raw_eh, filename, (do_chown==OWNSTG_CHAR)))
  {
   eh_append(eh, (do_chown==OWNSTG_CHAR)?OWNER_ID:OWNER_ID_NUM, raw_eh, get_owner_size(raw_eh));
   farfree(raw_eh);
  }
 ...
 lfn=0;
  ...
  else if(lfn_supported==LFN_COMP&&dual_name)
  {
   ...
   if(!file_exists(hdr_comment+FILENAME_MAX))
   {
    errors++;
    error_report();
    msg_printf(M_CANTOPEN, hdr_comment+FILENAME_MAX);
    nputlf();
    write_index_entry(idxid_fault);
    return(0);
   }
 ...
 addition_msg(is_update, is_replace, fix_longnames?ea_res:filename);
 ...
 else if(file_type==ARJT_BINARY||file_type==ARJT_TEXT)
 {
  if(file_type==ARJT_TEXT)
  {
   ...
   {
    msg_printf(M_REARCHIVING, filename);
    file_type=ARJT_BINARY;
    file_close(encstream);
    encstream=0L;
    if(open_input_file())
 ...
 if(verbose_display==VERBOSE_STD)
  msg_printf(M_N_BYTES, compsize);
 if(debug_enabled&&strchr(debug_opt, 't')!=NULL)
  msg_printf(M_N_TICKS, st_ticks=get_ticks()-st_ticks);
 ...
 if(!debug_enabled||strchr(debug_opt, 'x')==NULL)
 {
  msg_printf(M_FSTAT_2, ratio/10, ratio%10);
  #if TARGET==OS2
   if(ea_supported&&!volume_flag_set&&ea_size!=0)
    msg_printf(M_EA_STATS_STG, ea_size);
  #endif
...
int create_chapter_mark()
{
 ...
 msg_printf(M_ADDING);
 if(verbose_display==VERBOSE_STD)
  msg_printf(M_CHAPTER);
 msg_printf(M_LIST_FORM, format_filename(filename));
 calc_basic_hdr_size();
 arj_flags=0;
 entry_pos=0;
 cur_time_stamp(&ftime_stamp);
...
FILE_COUNT copy_archive()
{
 ...
 for(i=0; i<flist_main.files; i++)
 {
  ...
  msg_printf(M_PROCESSING_ARCHIVE, name);
  if(find_header(0, arc)<0L)
  {
   msg_printf(M_NOT_ARJ_ARCHIVE, name);
   nputlf();
   errors++;
  ...
  else
  {
   ...
   {
    msg_printf(M_ADDING);
    msg_printf(M_FILENAME_FORM, filename);
    write_index_entry(nullstr);
...
static int test_unpack_condition(char *name)
{
 ...
 if(!file_exists(name))
 {
   ...
   if(freshen_criteria!=FC_NONE||continued_prevvolume)
   {
     ...
     if(continued_prevvolume&&start_at_ext_pos&&ext_pos!=0L)
     {
      ...
      if(answer==0)
      {
       msg_fprintf(new_stdout, M_NOT_EXISTS, fname);
  ...
  {
   msg_fprintf(new_stdout, M_CANTOPEN, name);
   fprintf(new_stdout, ", ");
   return(2);
  }
 #endif
 ...
 {
  #if SFX_LEVEL>=ARJSFXV
   msg_fprintf(new_stdout, M_EXISTS, name);
  #else
   msg_printf(M_EXISTS, name);
  #endif
  return(1);
 }
 #if SFX_LEVEL>=ARJSFXV
 ...
 if(!skip_ts_check)
 {
   ...
   if(update_criteria==UC_NEW_OR_CRC||freshen_criteria==FC_CRC)
   {
    if(!volume_file&&fsize==origsize)
    {
     if(crc32_for_file(filename)==file_crc)
     {
      msg_fprintf(new_stdout, M_IS_SAME, fname);
      return(1);
     }
    }
   }
   ...
   else if(update_criteria==UC_NEW_OR_OLDER||freshen_criteria==FC_OLDER)
   {
    if(ts_cmp(&ftime, &ftime_stamp)<=0)
    {
     msg_fprintf(new_stdout, M_IS_SAME_OR_OLDER, fname);
     return(1);
    }
   }
   else if(update_criteria==UC_NEW_OR_NEWER||freshen_criteria==FC_EXISTING)
   {
    if(ts_cmp(&ftime, &ftime_stamp)>=0)
    {
     msg_fprintf(new_stdout, M_IS_SAME_OR_NEWER, fname);
     return(1);
    }
   }
  #else
   if(update_criteria!=UC_NONE&&freshen_criteria!=FC_NONE)
   {
    if(ts_cmp(&ftime, &ftime_stamp)>=0)
    {
      ...
      msg_fprintf(new_stdout, M_IS_SAME_OR_NEWER, fname);
     #else
      msg_printf(M_IS_SAME_OR_NEWER, name);
     #endif
     return(1);
    }
   }
  ...
  {
   msg_fprintf(new_stdout, M_IS_NOT_SAME_DATE, fname);
   return(2);
  }
 #endif
 #if SFX_LEVEL>=ARJ
 ...
 /* Multivolume preprocessing */
  ...
  #endif
   {
    if(skip_append_query||(file_type!=ARJT_BINARY&&file_type!=ARJT_TEXT))
     return(0);
    msg_fprintf(new_stdout, (!ts_cmp(&ftime_stamp, &ftime))?M_EXISTS:M_IS_NOT_SAME_DATE, fname);
    #if SFX_LEVEL>=ARJ
  ...
  msg_fprintf(new_stdout, M_ARJ_VS_DISK, origsize, arc_time+2, properties.fsize, disk_time+2);
  msg_fprintf(new_stdout, (ts_cmp(&ftime, &ftime_stamp)>=0)?M_IS_SAME_OR_NEWER:M_EXISTS, fname);
  return(query_action(REPLY_YES, QUERY_OVERWRITE, M_QUERY_OVERWRITE)?0:-1);
 #else
  msg_printf((ts_cmp(&ftime_stamp, &ftime)>0)?M_EXISTS:M_IS_SAME_OR_NEWER, name);
  msg_printf(M_QUERY_OVERWRITE);
...
#endif
{
 /* The text below is NOT a typo, this check is really used in ARJSFXV
  ...
  {
   msg_fprintf(new_stdout, M_UNKNOWN_VERSION, (int)arj_x_nbr);
   skip_file();
   return(-1);
  }
 #endif
 ...
 {
  #if SFX_LEVEL>=ARJSFXV
   msg_fprintf(new_stdout, M_FILE_IS_GARBLED);
  #else
   msg_printf(M_FILE_IS_GARBLED);
  #endif
  skip_file();
  return(-1);
 }
  ...
  {                                      /* ARJ 0.xx -m4 is not supported */
   msg_fprintf(new_stdout, M_UNKNOWN_METHOD, (int)method);
   skip_file();
   return(-1);
  }
  #if SFX_LEVEL>=ARJ
  ...
  #endif
  {
   msg_fprintf(new_stdout, M_UNKNOWN_FILE_TYPE, file_type);
   skip_file();
   return(-1);
  }
 #endif
 /* Check for ability to extract Windows 95 LFNs */
  ...
  if(disk_touched&&(host_os==OS_WIN95||host_os==OS_WINNT))
  {
   ...
   {
    msg_fprintf(new_stdout, M_REQUIRES_WIN95);
    skip_file();
    return(-1);
   }
  }
...
#endif
{
 int action;
  ...
  {
   action=BOP_NONE;
   msg_printf(M_TESTING, format_filename(filename));
   if(!verbose_display)
    fprintf(new_stdout, vd_space);
   else
   {
 ...
 unpack_file(action);
  ...
  if(action==BOP_COMPARE&&(method==8||test_archive_crc==TC_ATTRIBUTES))
  {
   ...
   if(ts_native(&ftime_stamp, OS)==properties.ftime&&origsize==properties.fsize)
    msg_printf(M_MATCHED);
   else
   {
    if(verbose_display)
    {
     if(ts_native(&ftime_stamp, OS)!=properties.ftime)
      msg_printf(M_BY_DATE);
     if(origsize!=properties.fsize)
      msg_printf(M_BY_SIZE);
    }
    msg_printf(M_NOT_MATCHED);
    if(errorlevel==ARJ_ERL_SUCCESS)
     errorlevel=ARJ_ERL_CRC_ERROR;
    errors++;
    nf_error=1;
  ...
  if(cmd==ARJ_CMD_WHERE)
  {
   for(pattern=0; pattern<SEARCH_STR_MAX; pattern++)
   {
    ...
    {
     nf_error=1;
     msg_sprintf(misc_buf, M_FOUND_N_OCCURENCES, search_occurences[pattern], search_str[pattern]);
     scroll_puts((FMSG *)misc_buf);
    }
   }
  }
...
static int extract_fn_proc()
{
 ...
 if(!file_exists(efn))
 {
  msg_printf(M_SKIPPED, efn);
  errors++;
  write_index_entry(idxid_fault);
  return(0);
 }
 if(show_filenames_only||!strcmp_os(filename, efn))
  msg_printf(M_RESTORING_PROPERTIES, efn);
 else
 {
  msg_printf(M_RESTORING_PROPERTIES, format_filename(filename));
  msg_printf(M_TO, format_filename(efn));
 }
 if(!test_host_os((int)host_os)&&file_type==ARJT_BINARY)
  msg_fprintf(new_stdout, M_FOREIGN_BINARY);
 nputlf();
...
#endif
{
 ...
 else
 {
   ...
   if(chk_free_space)
   {
    ...
    {
     #if SFX_LEVEL>=ARJ
      msg_fprintf(new_stdout, M_NOT_ENOUGH_SPACE);
     #else
   ...
   if(to_console)
   {
    ...
    if(!help_issued)
     msg_fprintf(new_stdout, M_EXTRACTING_1_TO_2, format_filename(filename), "STDOUT");
   }
   else
   {
  #endif
    if(file_type==ARJT_LABEL)
    {
     #ifdef HAVE_DRIVES
      #if SFX_LEVEL>=ARJ
       msg_printf(M_EXTRACTING, fname=format_filename(filename));
      #else
       msg_printf(M_EXTRACTING, format_filename(filename));
      #endif
      fprintf(new_stdout, vd_space);
...
cycle:;
      ...
      if(atstream==NULL)
      {
        ...
        error_report();
       #endif
       msg_printf(M_CANTOPEN, out_name);
       nputlf();
    ...
    else
    {
     ...
     msg_printf(M_EXTRACTING_1_TO_2, format_filename(filename), out_name);
    }
   #if SFX_LEVEL>=ARJ
    ...
    if(continued_prevvolume)
   #endif
     msg_printf(M_AT_POSITION_N, ftell(atstream));
  #if SFX_LEVEL>=ARJ
   }
  #endif
  #endif
 /* For ARJSFX, use a simplified algorithm... */
   ...
   if(file_type!=ARJT_DIR&&file_type!=ARJT_UXSPECIAL)
   {
    ...
    {
     msg_printf(M_CANTOPEN, tmp_tmp_filename);
     fputc(LF, stdout);
     skip_file();
     errors++;
     return(0);
   ...
   if(strcmp_os(filename, tmp_tmp_filename))
    msg_printf(M_EXTRACTING_1_TO_2, filename, tmp_tmp_filename);
   else
    msg_printf(M_EXTRACTING, filename);
   if(verbose_display)
   {
    if(file_type==ARJT_BINARY&&!test_host_os(host_os))
     msg_printf(M_FOREIGN_BINARY);
    else
     fputc(LF, stdout);
    msg_printf(method==0?M_UNSTORING:M_UNCOMPRESSING);
   }
  ...
  /* ...and the rest may be safely omitted for ARJSFX modules */
   ...
   if(!test_host_os((int)host_os)&&file_type==ARJT_BINARY)
    ...
    else
    {
     nputlf();
     if(method==0)
      msg_fprintf(new_stdout, M_UNSTORING);
     else
      msg_fprintf(new_stdout, M_UNCOMPRESSING, method);
     msg_fprintf(new_stdout, M_N_BYTES, origsize);
    }
   #else
    fprintf(new_stdout, vd_space);
   #endif
...
//    garble_init(password_modifier);
   ...
   else
   {
    ...
    if(volume_flag_set)
    {
     ...
     msg_fprintf(new_stdout, M_CONTINUED_NEXTVOL);
    }
    else
    {
     if(file_type==ARJT_BINARY||file_type==ARJT_TEXT)
     ...
     /* Create UNIX special files if able to */
       ...
       if((uxspec_rc=set_uxspecial(raw_eh, tmp_tmp_filename))!=0)
       {
        switch(uxspec_rc)
        {
         ...
         case UXSPEC_RC_FOREIGN_OS:
          msg_fprintf(new_stdout, M_FOREIGN_SYSTEM);
          break;
         case UXSPEC_RC_NOLINK:
          msg_fprintf(new_stdout, M_NO_LINKING);
          break;
 #if SFX_LEVEL>=ARJ          
         case UXSPEC_RC_SUPPRESSED:
          msg_fprintf(new_stdout, M_UXSPEC_SUPPRESSED);
          break;          
 #endif
#endif          
         default:
          msg_fprintf(new_stdout, M_CANT_SET_UXSPECIAL);
          break;
        }
        if(errorlevel==ARJ_ERL_SUCCESS)
      ...
      if(file_type==ARJT_UXSPECIAL)
      {
       msg_fprintf(new_stdout, M_UXSPECIAL_UNSUPP);
       fprintf(new_stdout, vd_space);
      }
     #endif
     /* Restore the file owner */
         ...
         )
      {
       ...
       {
        msg_fprintf(new_stdout, M_CANT_CHOWN);
        if(errorlevel==ARJ_ERL_SUCCESS)
         errorlevel=ARJ_ERL_WARNING;
       }
      }
     ...
     /* Deflate and store the extended attributes */
      ...
      if(ea_supported&&(p_eh=eh_lookup(eh, EA_ID))!=NULL)
      {
       ...
       {
        msg_fprintf(new_stdout, M_CANT_SET_EA);
        if(errorlevel==ARJ_ERL_SUCCESS)
         errorlevel=ARJ_ERL_WARNING;
       }
       else
       {
        ...
        if(ea_size>0)
        {
         msg_fprintf(new_stdout, M_EA_STATS_STG, ea_size);
         fprintf(new_stdout, vd_space);
        }
       }
       farfree(raw_eh);
   ...
   else
   {
    if(!print_with_more)
    {
     msg_fprintf(new_stdout, M_CRC_ERROR);
     if(errorlevel==ARJ_ERL_SUCCESS)
      errorlevel=ARJ_ERL_CRC_ERROR;
     errors++;
  ...
  for(i=0; i<flist_main.files; i++)
  {
  ...
  #endif
   {
    ...
    else
    {
     ...
     {
      if(total_chapters!=0&&current_chapter==RESERVED_CHAPTER&&(int)ext_flags==total_chapters&&(int)chapter_number==total_chapters)
       return(0);
      if(total_chapters!=0&&current_chapter==0&&(int)ext_flags<=total_chapters&&(int)chapter_number<total_chapters)
       return(0);
      if(total_chapters!=0&&current_chapter!=0&&current_chapter<=CHAPTERS_MAX&&(int)ext_flags<=chapter_to_process&&(int)chapter_number<current_chapter)
       return(0);
    #endif
...
int arcv_delete(FILE_COUNT num)
{
 ...
 if(del_action)
  msg_printf(M_DELETING, filename);
 total_files++;
 return(1);
}
