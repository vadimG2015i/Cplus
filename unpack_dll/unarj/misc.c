/*
 * $Id: misc.c,v 1.1.1.1 2002/03/28 00:03:10 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
void *malloc_far_str(char FAR *str)
{
 ...
 if ((k=malloc_msg(far_strlen(str)+1))==NULL )
 {
...
int flist_add(struct flist_root *root, struct flist_root *search_flist, char *name, FILE_COUNT *count, struct file_properties *properties)
{
 ...
 if(properties!=NULL&&root==&flist_main&&!match_attrib(properties))
 {
  ...
  if(properties!=NULL&&properties->hl_search.refcount>1&&!suppress_hardlinks)
  {
   for(i=0; i<hl_entries.total; i++)
   {
    /* The refcount values are not compared, since these may vary */
    if(!far_memcmp(&properties->hl_search.dev, (void FAR *)&hl_entries.list[i].dev, sizeof(dev_t))&&
       properties->hl_search.inode==hl_entries.list[i].inode)
    {
     properties->islink=1;
     properties->hl_search.ref=hl_entries.list[i].ref;
     properties->type=ARJT_UXSPECIAL;
     properties->fsize=0L;
     break;
    }
   }
   if(i==hl_entries.total)
   {
    if(hl_entries.total>=hl_entries.alloc)
    {
     hl_entries.alloc+=HL_ENTRIES_INCREMENT;
     hl_entries.list=(struct hl_search FAR *)farrealloc_msg(hl_entries.list, hl_entries.alloc*sizeof(struct hl_search));
    }
    hl_entries.list[i]=properties->hl_search;
    hl_entries.list[i].ref=root->files;
    hl_entries.total++;
   }
  }
 #endif
